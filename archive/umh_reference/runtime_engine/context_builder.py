...
class UnifiedContext:
    ...
    # Adaptive layer
    adaptive_directives: str | None = None
...
class ContextBuilder:
    def build(
        ...
        calibrated_thresholds: Any = None,
    ) -> UnifiedContext:
        uc = UnifiedContext()

        # Layer 0: AI Identity
        try:
            from umh.runtime_engine.ai_identity import AIIdentityEngine
        ...
        try:
            from umh.runtime_engine.signal_hierarchy import SignalHierarchyEngine

            she = SignalHierarchyEngine(ctx=ctx)
            uc.signal_classification = she.classify_input(
                message or "", channel="unknown"
            )
        ...
        try:
            from umh.runtime_engine.quality_gate import (
        ...
        try:
            from umh.runtime_engine.tenant import TenantManager
        ...
        try:
            ...
            if learnings:
                ...
                if lines:
                    uc.dex_learnings = "## What Antony Has Taught DEX\n" + "\n".join(
                        lines[:10]
                    )
        except Exception as e:
        ...
        try:
            from umh.runtime_engine.notebooklm_sync import NotebookLMSync
            ...
            if insights:
                ...
                if nlm_lines:
                    uc.notebooklm_insights = "NOTEBOOKLM INSIGHTS:\n" + "\n".join(
                        nlm_lines
                    )
        except Exception as e:
        ...
        # Layer 1f: Primitives
        try:
            from umh.runtime_engine.primitives import PrimitiveRegistry
        ...
        try:
            ...
            bis = bim.get_bis(venture_id or getattr(ctx, "active_venture_id", "") or "")
        ...
        # Confidentiality
        try:
            from umh.runtime_engine.confidentiality import detect_confidential_context
        ...
        # Martell patterns
        try:
            from umh.runtime_engine.martell_patterns import (
        ...
        # No List enforcement
        try:
            from umh.runtime_engine.founder_rate import check_against_no_list
        ...
        # Adaptive prompt layer — reads session evaluations + world model
        try:
            from umh.runtime_engine.adaptive_prompt import adapt_prompt

            _wm_for_adapt = None
            try:
                from umh.world.model import WorldModel as _WM

                _wm_for_adapt = _WM(org_id=str(ctx.org_id))
            except Exception:
                pass

            _base = uc.to_system_prompt()
            _adapted = adapt_prompt(
                base_prompt=_base,
                context={
                    "agent_type": agent,
                    "venture_id": venture_id or "",
                ...
                world_model=_wm_for_adapt,
                thresholds=calibrated_thresholds,
            )
            if _adapted != _base:
                _diff = _adapted[: len(_adapted) - len(_base)].strip()
                if _diff:
                    uc.adaptive_directives = _diff
        except Exception as e:
            uc.failed_sources.append(f"adaptive_prompt: {e}")

...
def _build_reason(outcome_type: str, attribution: float, confidence: float) -> str:
    ...
    attr_label = (
        "high" if attribution > 0.7 else "moderate" if attribution > 0.3 else "low"
    )
    conf_label = (
        "high" if confidence > 0.7 else "moderate" if confidence > 0.3 else "low"
    )
...
def credit_to_learning_signal(
    ...
    if (
        credit.outcome_type == "unknown"
        or credit.confidence < MIN_CONFIDENCE_FOR_LEARNING
    ):
    ...
    if trace is not None:
        ...
        if (
            credit.outcome_type == "failure"
            and mc_instability is not None
            and mc_instability > 0.3
        ):
...
def apply_signal_to_arbiter(
    ...
    if signal.confidence < MIN_CONFIDENCE_FOR_LEARNING:
        return False
    if (
        signal.reward_bias == 0.0
        and signal.risk_bias == 0.0
        and signal.stability_bias == 0.0
    ):
        return False
    ...
    try:
        from umh.runtime_engine.objective_arbitration import ObjectiveWeights, _clamp_weights

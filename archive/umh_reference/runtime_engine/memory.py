...
from umh.storage.adapters.neon import get_conn, resolve_venture, resolve_skill, ORG_ID, USER_ID
...
class AgentMemory:
            ...
            if _total > 0 and _total % 10 == 0:
                ...
                def _refresh_user_profile() -> None:
                    try:
                        ...
                        from umh.runtime_engine.user_model import UserModel as _UM
    ...
    def _fire_milestone_alert(outcome_count: int) -> None:
        ...
        def _alert() -> None:
            try:
                from umh.runtime_engine.orchestrator import check_outcome_milestone
                from umh.environments.system_context import load_context_from_env
    ...
    def store(
        ...
        try:
            ...
            with get_conn(self.ctx.org_id) as cur:
                ...
                # Embed and store
                try:
                    from umh.memory.embedder import embed
                    vec = embed(content)
                    cur.execute(

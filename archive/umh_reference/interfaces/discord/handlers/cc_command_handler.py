...
async def handle_followup(message, text: str) -> bool:
    ...
    try:
        ...
        await _send_reply(message.channel, f"❌ Error: {e}")
    return True
...
async def handle_travel(message, text: str) -> bool:
    ...
    try:
        ...
        if _result:
            await _send_reply(
                message.channel, f"🚗 Travel blocks created ({_minutes} min each side)."
            )
        else:
            await _send_reply(message.channel, "❌ Failed to create travel blocks.")
    except Exception as e:
        await _send_reply(message.channel, f"❌ Error: {e}")
    return True
    ...
    if _ch_id in pending_events:
        ...
        try:
            ...
            if _event:
                ...
                await _send_reply(message.channel, "❌ Failed to create event.")
        except Exception as e:
            await _send_reply(message.channel, f"❌ Error: {e}")
    else:
        await _send_reply(message.channel, "No pending event to confirm.")
    return True
...
async def handle_meetingroi(message, text: str) -> bool:
    ...
    try:
        from umh.runtime_engine.meetings import calculate_meeting_roi

        _roi = calculate_meeting_roi(venture=_venture, days=30)
        if not _roi or not _roi.get("total"):
            await _send_reply(
                message.channel, "📊 No meeting data for the last 30 days."
            )
            return True
        _lines = [
        ...
        await _send_reply(message.channel, "\n".join(_lines))
    except Exception as e:
        await _send_reply(message.channel, f"❌ Error: {e}")
    return True
...
async def handle_competitive(message, text: str) -> bool:
    ...
    try:
        from umh.runtime_engine.competitive_intel import synthesize_competitive_landscape

        _analysis = synthesize_competitive_landscape(_venture)
        await _send_reply(
            message.channel, f"🎯 **Competitive landscape — {_venture}:**\n{_analysis}"
        )
    except Exception as e:
        await _send_reply(message.channel, f"❌ Error: {e}")
    return True
...
async def handle_documents(message, text: str) -> bool:
    ...
    try:
        ...
        if not _rows:
            await _send_reply(
                message.channel, "📁 No documents filed in the last 30 days."
            )
            return True
...
async def handle_audit(message, text: str) -> bool:
    ...
    try:
        ...
        if not _rows:
            await _send_reply(
                message.channel, f"📋 No audit events in the last {_days} day(s)."
            )
            return True
        ...
        _full_audit = "\n".join(_lines)
        await _send_reply(message.channel, _full_audit)
    except Exception as e:
        await _send_reply(message.channel, f"❌ Audit failed: {e}")
    return True
...
async def handle_stakeholders(message, text: str) -> bool:
    ...
    _venture = _parts[1] if len(_parts) > 1 else None
    try:
        from umh.runtime_engine.stakeholder_map import (
            get_stakeholders,
...
async def handle_add_stakeholder(message, text: str) -> bool:
    ...
    if len(_parts) < 3:
        ...
        return True
    try:
        from umh.runtime_engine.stakeholder_map import add_stakeholder
        ...
        if _ok:
            await _send_reply(
                message.channel, f"✅ Stakeholder added: {_name} — {_venture} — {_role}"
            )
        else:
            await _send_reply(message.channel, "❌ Failed to add stakeholder.")
    except Exception as e:
        await _send_reply(message.channel, f"❌ Error: {e}")
    return True
...
async def handle_calendar_write(message, text: str, pending_events: dict) -> bool:
    ...
    try:
        ...
        _cal_raw = utility_llm_call(prompt=_cal_prompt, operation="calendar_extract")
        _parsed = json.loads(_cal_raw.strip())
        ...
        if _action == "create":
            ...
            if _events:
                ...
                await _send_reply(message.channel, "📅 No upcoming events found.")
            return True

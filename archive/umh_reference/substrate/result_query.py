...
from umh.substrate.result_store import (
    IngestedResult,
    ResultStore,
    get_result_store,
)
...
def unresolved_rituals(limit: int = 20) -> list[dict]:
    ...
    """
    try:
        from umh.substrate.rituals import RitualRegistry
    except Exception:
        return []
    s = get_result_store()
    try:
...
def station_readiness_report(node_id: str) -> dict[str, Any]:
    ...
    """
    try:
        from umh.substrate.scene_policy import select_scene
        from umh.substrate.station_readiness import station_readiness
    ...
    try:
        from umh.substrate.ritual_inference import infer_open_scene_hint
    ...
    try:
        from umh.substrate.scene_capabilities import scene_requirements_inventory
...
def recent_open_close_summaries(limit: int = 5) -> list[dict]:
    ...
    """
    try:
        from umh.substrate.rituals import RitualKind, RitualRegistry
    except Exception:
        return []
    try:
...
def recent_voice_sessions(
    ...
    """
    try:
        from umh.substrate.voice_session import get_voice_session_store
    except Exception:
        return []
    try:
...
def recent_wake_producer_events(
    ...
    *,
    ...
    """
    try:
        from umh.substrate.wake_producer import get_wake_producer_history
    except Exception:
        return []
    try:
...
def operator_state_snapshot(
    ...
    """
    try:
        from umh.substrate.operator_state import get_operator_state_store
    except Exception:
...
def audio_loop_snapshot(
    ...
    """
    try:
        from umh.substrate.audio_loop import snapshot as _audio_snapshot
    except Exception:
...
def recent_audio_loop_transcripts(
    ...
    """
    try:
        from umh.substrate.audio_loop import get_audio_loop_store
    except Exception:
        return []
    try:

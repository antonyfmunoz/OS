...
How it works:
  - EOS writes dispatched SafeActions to a per-node JSON outbox file:
        /opt/OS/umh/.substrate_station/<node_id>.outbox.json
  - The local Station Daemon (running on the founder's workstation, either
    ...
    in production) polls the outbox, executes, writes results/events to:
        /opt/OS/umh/.substrate_station/<node_id>.inbox.json
...
from umh.substrate.actions import SafeAction, ActionResult, ActionStatus
from umh.substrate.station import StationEvent


_BUS_DIR = Path("/opt/OS/umh/.substrate_station")

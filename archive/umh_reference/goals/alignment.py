...
Usage::
    ...
    from umh.goals.alignment import GoalAlignmentEvaluator, AlignmentResult
...
if TYPE_CHECKING:
    ...
    from umh.goals.meta_goal import MetaGoal
...
class GoalAlignmentEvaluator:
    ...
    def evaluate_alignment(
        ...
        if consistency_score < CONSISTENCY_PENALTY_THRESHOLD:
            penalties.append(f"consistency:unstable_delta_history:score={consistency_score:.2f}")

        # ── Signal B: Outcome-adjusted utility ─────────────────────
        outcome_score = self._score_outcome_utility(meta_goal, traces)
        if outcome_score < OUTCOME_PENALTY_THRESHOLD:
            penalties.append(f"outcome:poor_outcome_correlation:score={outcome_score:.2f}")
        ...
        if efficiency_score < EFFICIENCY_PENALTY_THRESHOLD:
            penalties.append(f"efficiency:high_cost_low_return:score={efficiency_score:.2f}")

        # ── Signal D: Conflict detection ───────────────────────────
        conflict_score = self._score_conflict(meta_goal, registry, traces)
        if conflict_score < 0.5:
            penalties.append(f"conflict:negative_impact_on_peers:score={conflict_score:.2f}")
        ...
        from umh.goals.validator import PRIORITY_MIN

...
Usage::
    ...
    from umh.goals.arbitrator import GoalArbitrator
...
def _stable_softmax(values: list[float], temperature: float = SOFTMAX_TEMPERATURE) -> list[float]:
...
class GoalArbitrator:
    ...
    def select_active_goal(
        ...
        _switch_applied = previous_active_goal_id is not None and best_id != previous_active_goal_id
        _reason = "max_utility"
        if _switch_applied:

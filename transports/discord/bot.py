...
def _run_substrate(
    ...
    """Route through substrate instead of gateway. Same signature as _run_gateway for drop-in replacement."""
    import asyncio as _sub_asyncio
    from unittest.mock import MagicMock

    # Build a minimal mock message for the signal factory
    msg = MagicMock()
    msg.content = text
    msg.author.id = username
    msg.author.name = username
    msg.guild = MagicMock()
    msg.guild.id = guild_id or "0"
    ...
    try:
        loop = _sub_asyncio.new_event_loop()
        result = loop.run_until_complete(_substrate.execute(signal))
        loop.close()
        return result.output or "I'm processing your request."
    except Exception as e:
        _record_error("substrate_execute", e)
        # Fall back to gateway on substrate failure
        return _run_gateway(text, channel_name, username, guild_id, channel_id, memory_only)
...
async def on_message(message: discord.Message):
    ...
    # All messages → substrate routing (replaces gateway for general text path)
    ...
    async with message.channel.typing():
        ...
        output = await loop.run_in_executor(
            None,
            lambda: _run_substrate(text, channel_name, username, guild_id=_gid, channel_id=_cid),
        )

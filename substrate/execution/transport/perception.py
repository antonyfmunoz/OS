...
def collect_task_perception() -> list[PerceptionRecord]:
    ...
    try:
        from substrate.execution.transport.task_system import TaskStore, TaskStatus
...
def collect_pipeline_perception() -> list[PerceptionRecord]:
    ...
    records: list[PerceptionRecord] = []
    try:
        from substrate.execution.transport.task_pipeline import PipelineStore, PipelineStatus
...
def collect_node_status_perception() -> list[PerceptionRecord]:
    ...
    records: list[PerceptionRecord] = []
    try:
        from substrate.execution.transport.nodes import NodeRegistry, NodeStatus, NodeType
...
def collect_station_presence_perception() -> list[PerceptionRecord]:
    ...
    records: list[PerceptionRecord] = []
    try:
        from substrate.execution.transport.station_presence import get_station_presence, StationPresenceMode
        ...
        if state.mode == StationPresenceMode.OVERNIGHT:
            try:
                from substrate.execution.transport.operator_session import OperatorSessionStore
...
def collect_local_control_perception() -> list[PerceptionRecord]:
    ...
    records: list[PerceptionRecord] = []
    try:
        from substrate.execution.transport.local_control import (
            LocalControlStore,
            RequestStatus,
        )
...
def collect_live_session_perception() -> list[PerceptionRecord]:
    ...
    records: list[PerceptionRecord] = []
    try:
        from substrate.execution.transport.live_sessions import (
            LiveSessionStore,
            LiveSessionState,
        )
        from datetime import timedelta

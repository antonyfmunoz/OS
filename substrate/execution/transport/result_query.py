...
from substrate.execution.transport.result_store import (
    IngestedResult,
    ResultStore,
    get_result_store,
)
...
def unresolved_rituals(limit: int = 20) -> list[dict]:
    ...
    """
    try:
        from substrate.execution.transport.rituals import RitualRegistry
    except Exception:
        return []
    s = get_result_store()
    try:
...
def station_readiness_report(node_id: str) -> dict[str, Any]:
    ...
    """
    try:
        from substrate.execution.transport.scene_policy import select_scene
        from substrate.execution.transport.station_readiness import station_readiness
    ...
    try:
        from substrate.execution.transport.ritual_inference import infer_open_scene_hint
    ...
    try:
        from substrate.execution.transport.scene_capabilities import scene_requirements_inventory
...
def recent_open_close_summaries(limit: int = 5) -> list[dict]:
    ...
    """
    try:
        from substrate.execution.transport.rituals import RitualKind, RitualRegistry
    except Exception:
        return []
    try:
...
def recent_voice_sessions(
    ...
    """
    try:
        from substrate.execution.transport.voice_session import get_voice_session_store
    except Exception:
        return []
    try:
...
def recent_wake_producer_events(
    ...
    *,
    ...
    """
    try:
        from substrate.execution.transport.wake_producer import get_wake_producer_history
    except Exception:
        return []
    try:
...
def operator_state_snapshot(
    ...
    """
    try:
        from substrate.execution.transport.operator_state import get_operator_state_store
    except Exception:
...
def audio_loop_snapshot(
    ...
    """
    try:
        from substrate.execution.transport.audio_loop import snapshot as _audio_snapshot
    except Exception:
...
def recent_audio_loop_transcripts(
    ...
    """
    try:
        from substrate.execution.transport.audio_loop import get_audio_loop_store
    except Exception:
        return []
    try:

"""
execution.transport — Lazy-import package.

All symbols are available via ``from substrate.execution.transport import X`` or
``from substrate.execution.transport.submodule import Y``. Submodules are loaded
...
Migrated from runtime.transport → execution.transport (2026-05-14, §24).
"""
...
def _m(submodule: str, *names: str) -> None:
    ...
    for name in names:
        _LAZY_MAP[name] = (f"substrate.execution.transport.{submodule}", name)
...
def __getattr__(name: str) -> Any:
    ...
    raise AttributeError(f"module 'execution.transport' has no attribute {name!r}")

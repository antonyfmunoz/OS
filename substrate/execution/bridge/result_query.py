...
from substrate.execution.bridge.result_store import (
    IngestedResult,
    ResultStore,
    get_result_store,
)
...
def unresolved_rituals(limit: int = 20) -> list[dict]:
    ...
    """
    try:
        from substrate.execution.bridge.rituals import RitualRegistry
    except Exception:
        return []
    s = get_result_store()
    try:
...
def station_readiness_report(node_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        from substrate.execution.bridge.station_readiness import station_readiness
    ...
    try:
        from substrate.execution.bridge.ritual_inference import infer_open_scene_hint
    ...
    try:
        from substrate.execution.bridge.scene_capabilities import scene_requirements_inventory
...
def recent_voice_sessions(
    ...
    """
    try:
        from substrate.execution.bridge.voice_session import get_voice_session_store
    except Exception:
        return []
    try:
...
def audio_loop_snapshot(
    ...
    """
    try:
        from substrate.execution.bridge.audio_loop import snapshot as _audio_snapshot
    except Exception:
...
def recent_audio_loop_transcripts(
    ...
    """
    try:
        from substrate.execution.bridge.audio_loop import get_audio_loop_store
    except Exception:
        return []
    try:

        ...
        try:
            ...
            for d in devices:
                ...
                if new_ts_name and d.get("tailscale_name") == new_ts_name:
                    raise ValueError(
                        f"Device with tailscale_name '{new_ts_name}' already exists"
                    )
    ...
    with open(lock_path, "w") as lock_fd:
        ...
        try:
            ...
            if target.get("always_online"):
                raise ValueError(
                    f"Cannot remove always_online device '{device_id}'"
                )
...
def invalidate_all_caches() -> None:
    ...
    try:
        ...
        logger.debug("Invalidated %d DeviceAwarenessRuntime instances",
                      len(device_awareness._INSTANCES))
    except Exception as exc:

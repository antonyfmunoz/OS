...
class ContradictionReport:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        for c in self.contradictions:
            ...
            type_counts[c.contradiction_type.value] = (
                type_counts.get(c.contradiction_type.value, 0) + 1
            )
        return {
...
def _check_route_panel_mismatch(report: ContradictionReport, world_model) -> None:
    ...
    for panel in panels:
        ...
        has_route = any(panel_name in rn or rn in panel_name for rn in route_names)
        if not has_route and panel_name not in (
            "dashboard",
            "editor",
            "settings",
            "comms",
...
def persist_contradictions(report: ContradictionReport, path: str | None = None) -> str:
    ...
    if path is None:
        ...
        path = str(runtime_state_path("organism", "contradictions.jsonl", create_parent=False))

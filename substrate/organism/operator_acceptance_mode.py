"""Operator acceptance mode — standard multi-runtime vs deterministic-only vs blocked.
...
_DETERMINISTIC_PROVES: list[str] = [
    "full operator loop plumbing: operator→AI→WorkPacket→context→permission→propagation→runtime→artifact→review",
...
class OperatorAcceptanceMode(str, Enum):
...
@dataclass
class OperatorAcceptanceModeDecision:
    """Captures the mode selection for a single acceptance run."""

    decision_id: str  # "oamd-<8hex>"
    mode: OperatorAcceptanceMode
    ...
    cloud_api_available: bool = False
    cloud_api_status: str = ""
    llm_cloud_provider_available: bool = False
    degraded: bool = False
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "selected_runtime_reason": self.selected_runtime_reason,
            "cloud_api_available": self.cloud_api_available,
            "cloud_api_status": self.cloud_api_status,
...
def to_dict(decision: OperatorAcceptanceModeDecision) -> dict[str, Any]:
    """Module-level convenience — delegates to the dataclass method."""
    return decision.to_dict()


def from_dict(data: dict[str, Any]) -> OperatorAcceptanceModeDecision:
    """Reconstruct a decision from a plain dictionary."""
    mode_raw = data["mode"]
    if mode_raw == "standard":
        mode_raw = "standard_multi_runtime"
    # Accept legacy "jamd-" prefix IDs
    decision_id = data.get("decision_id", f"oamd-{uuid4().hex[:8]}")
    return OperatorAcceptanceModeDecision(
        decision_id=decision_id,
        mode=OperatorAcceptanceMode(mode_raw),
        ...
        selected_runtime_reason=data.get("selected_runtime_reason", ""),
        cloud_api_available=data.get("cloud_api_available", False),
        cloud_api_status=data.get("cloud_api_status", ""),
...
def create_standard_mode_decision(
    ...
    """
    return OperatorAcceptanceModeDecision(
        decision_id=f"oamd-{uuid4().hex[:8]}",
        mode=OperatorAcceptanceMode.STANDARD_MULTI_RUNTIME,
        accepted_by_operator=operator_accepted,
        capable_runtime_path_exists=True,
        selected_runtime=selected_runtime,
        selected_device=selected_device,
        selected_runtime_reason=selected_runtime_reason or f"{selected_runtime} selected as primary governed runtime",
        cloud_api_available=llm_cloud_available,
        cloud_api_status=cloud_api_status or ("available" if llm_cloud_available else "exhausted"),
...
def create_deterministic_mode_decision(
    ...
    return OperatorAcceptanceModeDecision(
        decision_id=f"oamd-{uuid4().hex[:8]}",
        mode=OperatorAcceptanceMode.DETERMINISTIC_ONLY,
        accepted_by_operator=operator_accepted,
...
def select_acceptance_mode(
    ...
    return OperatorAcceptanceModeDecision(
        decision_id=f"oamd-{uuid4().hex[:8]}",
        mode=OperatorAcceptanceMode.BLOCKED,
...
def persist_mode_decision(
    decision: OperatorAcceptanceModeDecision,
...
def load_mode_decisions(
    persist_dir: str | None = None,
) -> list[OperatorAcceptanceModeDecision]:
    ...
    decisions: list[OperatorAcceptanceModeDecision] = []

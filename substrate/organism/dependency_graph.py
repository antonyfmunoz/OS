...
class DependencyGraph:
    ...
    def weak_dependencies(self) -> list[DependencyEdge]:
        ...
        return [
            e
            for e in self.edges
            if e.strength in (DependencyStrength.SOFT, DependencyStrength.OPTIONAL)
        ]
    ...
    def missing_dependencies(self) -> list[dict[str, str]]:
        ...
        for e in self.edges:
            if e.target not in known:
                missing.append(
                    {"source": e.source, "missing_target": e.target, "type": e.dep_type.value}
                )
            if e.source not in known:
                missing.append(
                    {"missing_source": e.source, "target": e.target, "type": e.dep_type.value}
                )
        return missing
...
_KNOWN_DEPENDENCIES: list[tuple[str, str, DependencyType, DependencyStrength, str]] = [
    ...
    (
        ...
        DependencyType.DATA,
        DependencyStrength.HARD,
        "LeverageAssimilator feeds into leverage engine",
    ),
    (
        "leverage_assimilation",
        "event_spine",
        DependencyType.EVENT,
        DependencyStrength.SOFT,
        "LeverageAssimilator emits assimilation events",
    ),
    (
        "advisor_hierarchy",
        "advisor",
        DependencyType.RUNTIME,
        DependencyStrength.HARD,
        "AdvisorHierarchy extends advisor with multi-level structure",
    ),
    (
        "advisor_hierarchy",
        "event_spine",
        DependencyType.EVENT,
        DependencyStrength.SOFT,
        "AdvisorHierarchy emits hierarchy events",
    ),
    (
        "transport_discord_bot",
        "adapter_model_router",
        DependencyType.RUNTIME,
        DependencyStrength.HARD,
            ...
            if dep_id in known_nodes and not any(
                ...
                graph.add_edge(
                    DependencyEdge(
                        ...
                        evidence="Declared dependency from WorldModel",
                    )
                )
...
def persist_dependency_graph(graph: DependencyGraph, path: str | None = None) -> str:
    ...
    if path is None:
        ...
        path = str(runtime_state_path("organism", "dependency_graph.jsonl", create_parent=False))

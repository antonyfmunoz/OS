...
VALID_DEAL_STAGES = frozenset({"discovery", "proposal", "negotiation", "closed-won", "closed-lost"})
...
def update_deal_stage(conn: Any, params: dict[str, Any]) -> dict[str, Any]:
    ...
    query = (
        f"UPDATE crm_deals SET {', '.join(set_clauses)} WHERE id = %s AND user_id = %s RETURNING id"
    )

...
class InstanceWorldModel:
    ...
    def propose_promotion(self, entry_id: str) -> bool:
        ...
        for key in self._store.all_keys():
            if key.startswith(f"worldmodel:instance:{self.org_id}:") and key.endswith(
                f":{entry_id}"
            ):
                data = self._store.get(key)
                if data:
...
WRITE_CONFIDENCE_THRESHOLD = 0.4
...
class WorldModel:
    ...
    def update_from_interaction(
        self, message: str, response: str, outcome: str | None = None
    ) -> bool:
        """Extract learnings from an interaction and store in instance model.

        Write-throttled: only persists if confidence >= WRITE_CONFIDENCE_THRESHOLD
        AND the content is not a duplicate of an existing entry (keyword overlap).

        Returns True if an entry was written, False if throttled.
        """
        entry_type = "pattern"
        if any(
            w in message.lower()
            for w in ["because", "causes", "leads to", "results in"]
        ):
            entry_type = "causal"
        elif any(
            w in message.lower() for w in ["should", "strategy", "approach", "plan"]
        ):
            entry_type = "strategy"
    ...
    def _is_duplicate(self, content: str) -> bool:
        """Check if content overlaps significantly with recent instance entries."""
        _stop = frozenset(
            {
                "a",
                "an",
        ...
        new_words = set(content.lower().split()) - _stop
        if not new_words:
            return False

        existing = self.instance.get_entries()
        for entry in existing[-20:]:
            existing_words = set(entry.content.lower().split()) - _stop
            if not existing_words:
                continue
            overlap = len(new_words & existing_words) / max(
                len(new_words | existing_words), 1
            )
    ...
    def get_context_for_prompt(self, query: str) -> str:
        ...
        if canonical:
            ...
            for entry in canonical:
                lines.append(
                    f"[{entry.entry_type}|{entry.confidence:.0%}] {entry.content[:200]}"
                )
            parts.append("\n".join(lines))
        ...
        relevant = (
            [e for e in instance if query.lower().split()[0] in e.content.lower()]
            if query.strip()
            else []
        )
        if relevant:
...
if __name__ == "__main__":
    ...
    wm.update_from_interaction(
        "test message about strategy", "test response about approach"
    )

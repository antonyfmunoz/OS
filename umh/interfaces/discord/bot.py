...
from umh.runtime_engine.voice_engine import VoiceEngine
from umh.workstation.business import get_ai_name
from umh.runtime_engine.discord_utils import chunk_message, post_to_webhook
from umh.substrate.session_discord_bridge import send_reply as _send_reply
from umh.substrate.discord_text_transport import (
    maybe_mirror_discord_text_message as _maybe_pseudo_live_text,
)
from umh.substrate.message_framing import get_inbound_buffer
from umh.substrate.event_spine import (
    EventType as _FrameEventType,
    create_event as _frame_create_event,
)
from umh.substrate.event_store import get_event_store as _frame_get_event_store
...
# routes through umh.model_router.call_with_fallback instead of the
...
try:
    from umh.substrate.voice_eos_responder import (
    ...
    if not _is_voice_router_responder_installed():
        ...
        print(
            ...
            "(umh.voice_eos_responder → model_router.call_with_fallback)",
            flush=True,
        )
    else:
        print(
...
from umh.runtime_engine.onboarding_engine import OnboardingEngine as _OnboardingEngine
...
def _run_day_command(
    ...
    *,
    ...
    try:
        from umh.substrate.day_workflows import close_day, open_day
...
async def handle_meeting_voice(
    ...
    if any(
        ...
        result = await loop.run_in_executor(
            None,
            lambda: __import__(
                "umh.runtime_engine.agent_teams", fromlist=["run_team_task"]
            ).run_team_task(
                team="sales",
...
async def _warmup_cc_sdk():
    ...
    try:
        from umh.runtime_engine.cc_sdk import query_cc_sync
...
async def on_ready():
    ...
    try:
        from umh.runtime_engine.orchestrator import start_ambient_refresh_loop
    ...
    try:
        ...
        from umh.substrate.session_watcher import start_watcher
    ...
    try:
        from umh.substrate.station_daemon import start_station_daemon
...
async def on_message(message: discord.Message):
    ...
    _orch_handled = False
    try:
        from umh.substrate.discord_ingress_adapter import ingest_and_emit
    ...
    _cc_injected = False
    try:
        from umh.substrate.discord_mode_routing import resolve_discord_mode
        from umh.runtime_loop.session_registry import get_registry
        from umh.runtime_loop.session_router import route_session_message
        from umh.runtime_loop.surface_registry import get_surface_registry
        from umh.substrate.target_policy import resolve_execution_policy
        ...
        try:
            from umh.runtime_loop.live_loop import get_or_create_runtime
    ...
    if _channel_id in _pending_captures and text.strip() in (
        ...
        try:
            from umh.runtime_engine.founder_capture import capture
    ...
    try:
        from umh.runtime_engine.founder_capture import should_capture, capture
...
async def cmd_watcher_status(ctx: commands.Context):
    ...
    try:
        ...
        from umh.substrate.session_watcher import _WATCHERS, _WATCHERS_LOCK

        with _WATCHERS_LOCK:
            if not _WATCHERS:
                await ctx.reply("No watchers running.")
                return
            from umh.substrate.discord_output_policy import get_display_name
...
async def cmd_status(ctx: commands.Context):
    ...
    async with ctx.typing():
        ...
        def _portfolio_scan():
            try:
                from umh.runtime_engine.portfolio_advisor import PortfolioAdvisor as PortfolioAgent
...
async def cmd_outcome(ctx: commands.Context, *, args: str = ""):
    ...
    try:
        ...
        from umh.runtime_engine.meetings import update_meeting_outcome
...
async def cmd_accept(ctx: commands.Context, event_id: str = ""):
    ...
    if not event_id:
        ...
        return
    try:
        from tools.calendar_invite_handler import respond_to_invite
...
async def cmd_approve_followup(ctx: commands.Context):
    ...
    try:
        ...
        from umh.runtime_engine.quality_gate import gate_outgoing_email
        import json as _json
...
async def cmd_confidential(ctx: commands.Context, *, args: str = ""):
    ...
    if not parts or not parts[0]:
        ...
        return
    try:
        from umh.runtime_engine.confidentiality import create_confidential_session
...
async def cmd_nurture(ctx: commands.Context, *, name: str = ""):
    ...
    if not name:
        ...
        return
    try:
        from umh.runtime_engine.person_recognition import build_intelligence_profile
        ...
        draft = utility_llm_call(
            prompt=f"""Draft a warm check-in message for {name}.
...
async def cmd_expenses(ctx: commands.Context):
    ...
    try:
        from umh.runtime_engine.expense_tracker import get_monthly_summary
...
async def cmd_report(ctx: commands.Context, report_type: str = "profile"):
    ...
    if report_type == "profile":
        ...
        results = await loop.run_in_executor(
            None,
            lambda: (
                __import__("umh.runtime_engine.world_pulse", fromlist=["WorldPulse"])
                .WorldPulse(_ctx_eos)
                .run_pulse_scan()
            ),
        )
...
async def cmd_sync(ctx: commands.Context):
    ...
    async with ctx.typing():
        ...
        def _run():
            try:
                from umh.runtime_engine.daily_sync import DailySyncEngine
...
async def cmd_delegated(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.delegation_tracker import get_overdue_delegations
...
async def cmd_subscriptions(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.subscription_tracker import (
                get_subscriptions,
...
async def cmd_add_sub(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.subscription_tracker import add_subscription
...
async def cmd_drip(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.task_yield_matrix import run_drip_audit, format_drip_report
...
async def cmd_buyback(ctx: commands.Context, income: str = ""):
    ...
    if income.strip():
        ...
        def _run():
            try:
                from umh.runtime_engine.founder_rate import (
        ...
        def _get():
            try:
                from umh.runtime_engine.founder_rate import get_current_buyback_rate
...
async def cmd_logtime(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.founder_rate import log_time_block
...
async def cmd_timeaudit(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.founder_rate import get_time_audit_summary
...
async def cmd_perfectweek(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.ideal_week import get_perfect_week
...
async def cmd_camcorder(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.ideal_week import create_camcorder_playbook
...
async def cmd_trip(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.travel_manager import build_travel_brief, log_trip
...
async def cmd_nolist(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.founder_rate import get_no_list
...
async def cmd_noadd(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.founder_rate import add_to_no_list
...
async def cmd_year(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.ideal_week import get_preloaded_year
...
async def cmd_rocks(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.ideal_week import get_current_quarter_rocks
...
async def cmd_invoices(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.expense_tracker import get_invoices, get_overdue_invoices
...
async def cmd_invoice(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.expense_tracker import create_invoice, generate_invoice_text
...
async def cmd_expensereport(ctx: commands.Context, month: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.expense_tracker import generate_expense_report
...
async def cmd_budget(ctx: commands.Context, target: str = "10000"):
    ...
    def _run():
        try:
            from umh.runtime_engine.expense_tracker import generate_budget_vs_actual
...
async def cmd_briefdoc(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.doc_creator import create_briefing_doc
...
async def cmd_slides(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.doc_creator import create_presentation_outline
...
async def cmd_factcheck(ctx: commands.Context, *, claim: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.doc_creator import fact_check
...
async def cmd_dates(ctx: commands.Context):
    ...
    def _run():
        try:
            from umh.runtime_engine.personal_admin import get_upcoming_dates
...
async def cmd_adddate(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.personal_admin import add_important_date
...
async def cmd_gift(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.personal_admin import research_gift
...
async def cmd_flights(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.travel_manager import research_flights
...
async def cmd_hotels(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.travel_manager import research_hotels
...
async def cmd_restaurants(ctx: commands.Context, *, args: str = ""):
    ...
    def _run():
        try:
            from umh.runtime_engine.travel_manager import research_restaurants
...
async def cmd_proofread(ctx: commands.Context, *, content: str = ""):
    ...
    if not content:
        ...
        return
    try:
        from umh.runtime_engine.quality_gate import quality_check
...
async def cmd_minutes(ctx: commands.Context, *, args: str = ""):
    ...
    if "|" not in args:
        ...
        return
    try:
        from umh.runtime_engine.meetings import draft_meeting_minutes
...
async def cmd_okr(ctx: commands.Context, subcommand: str = "report", *, args: str = ""):
    ...
    if subcommand == "report":
        try:
            from umh.runtime_engine.okr_tracker import generate_okr_report
        ...
        if not args or "|" not in args:
            ...
            return
        try:
            from umh.runtime_engine.okr_tracker import set_okr
...
async def cmd_event(ctx: commands.Context, *, args: str = ""):
    ...
    if not args or args.strip() == "list":
        try:
            from umh.runtime_engine.event_manager import get_events
    ...
    if "|" in args:
        parts = [p.strip() for p in args.split("|")]
        try:
            from umh.runtime_engine.event_manager import create_event
...
async def cmd_talkingpoints(ctx: commands.Context, *, args: str = ""):
    ...
    if len(parts) < 2:
        ...
        return
    try:
        from umh.runtime_engine.event_manager import draft_talking_points
...
async def cmd_pr(ctx: commands.Context, *, args: str = ""):
    ...
    if "|" not in args:
        ...
        return
    try:
        from umh.runtime_engine.event_manager import log_pr_media_inquiry
...
async def cmd_board_update(ctx: commands.Context, venture_id: str = ""):
    ...
    if not venture_id:
        ...
        return
    try:
        from umh.runtime_engine.stakeholder_map import generate_board_update_brief
...
async def cmd_announce(ctx: commands.Context, *, args: str = ""):
    ...
    if len(parts) < 3:
        ...
        return
    try:
        from umh.runtime_engine.doc_creator import draft_announcement
...
async def cmd_crisis(ctx: commands.Context, *, args: str = ""):
    ...
    if len(parts) < 3:
        ...
        return
    try:
        from umh.runtime_engine.doc_creator import draft_crisis_communication
...
async def cmd_itinerary(ctx: commands.Context, *, args: str = ""):
    ...
    if len(parts) < 3:
        ...
        return
    try:
        from umh.runtime_engine.travel_manager import generate_trip_itinerary
...
async def cmd_trace(ctx: commands.Context, limit: int = 5):
    ...
    try:
        ...
        from umh.substrate.execution_trace import (

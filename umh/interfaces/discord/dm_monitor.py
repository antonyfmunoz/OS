...
from umh.runtime_engine.error_handler import ErrorHandler
...
# TODO: Route through model_router once it supports vision/image analysis.
# This is a KNOWN BYPASS — direct genai.Client for vision tasks only.
# Tracked as a model_router gap, not a quick fix.
_gemini_client = None  # lazy-init, not module-scope


def _get_gemini_client():
    """Lazy-init Gemini client for vision tasks with error handling."""
    global _gemini_client
    if _gemini_client is not None:
        return _gemini_client
    if not _GENAI_AVAILABLE:
        print("[dm_monitor] google.genai not installed — vision fallback unavailable")
        return None
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        print("[dm_monitor] GEMINI_API_KEY not set — vision fallback unavailable")
        return None
    try:
        _gemini_client = genai.Client(api_key=api_key)
        print("[dm_monitor] Gemini vision client initialized (known bypass — needs model_router vision support)")
        return _gemini_client
    except Exception as e:
        print(f"[dm_monitor] Gemini client init failed: {e}")
        return None


# Backward compat alias — callers that reference gemini_client directly
gemini_client = None  # replaced by _get_gemini_client() calls
...
def generate_reply(conversation_text):
    ...
    try:
        ...
        _raw = utility_llm_call(
            prompt=_prompt,
            system=system,
            operation="dm_reply_draft",
        )
        return _raw.strip() if _raw else "What made you reach out today?"
    except Exception as e:
        print(f"[dm_monitor] UMH call failed: {e}")

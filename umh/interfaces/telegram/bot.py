...
# kpi_tracker and cost_tracker live in tools/
...
try:
    ...
    from umh.workstation.business import get_ai_name as _get_ai_name
...
def build_briefing_text():
    ...
    revenue_path = os.path.join(VAULT, "services/revenue_log.json")
    try:
        with open(revenue_path) as f:
            rev = json.load(f)
        month = datetime.date.today().isoformat()[:7]
        month_rev = rev["monthly"].get(month, 0.0)
        clients = len([c for c in rev["clients"] if c["date"].startswith(month)])
        rev_line = f"Revenue this month: ${month_rev:.0f} ({clients} clients)"
    except Exception:
...
async def scheduled_signal_scan(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        total_critical = sum(
            v.get("CRITICAL", 0) if isinstance(v, dict) else 0 for v in summary.values()
        )
        total_high = sum(
            v.get("HIGH", 0) if isinstance(v, dict) else 0 for v in summary.values()
        )
        if total_critical > 0 or total_high > 0:
            ...
            for venture_id, tiers in summary.items():
                if (
                    ...
                    and (tiers.get("CRITICAL", 0) + tiers.get("HIGH", 0)) > 0
                ):
                    lines.append(
...
_COMMAND_ALLOWLIST: dict[str, list[str]] = {
...
async def run_command(command: str, update: Update):
    """Execute a command from a strict allowlist. No shell=True, no arbitrary input."""
    cmd_argv = _COMMAND_ALLOWLIST.get(command.strip())
    if cmd_argv is None:
        allowed = ", ".join(sorted(_COMMAND_ALLOWLIST.keys()))
        await update.message.reply_text(
            f"Command not in allowlist. Allowed commands:\n{allowed}"
        )
        return
    result = subprocess.run(
        cmd_argv, capture_output=True, text=True, cwd=VAULT
    )
    output = (
        result.stdout.strip() or result.stderr.strip() or "Done. No output returned."
    )
    if len(output) > 4096:
        output = output[:4090] + "\n..."
...
async def blacklist_tag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text(
            "Usage: /blacklist <tag>  e.g. /blacklist discipline"
        )
        return
    ...
    if removed_from:
        await update.message.reply_text(
            f"#{tag} removed from group(s) {', '.join(removed_from)} and blacklisted."
        )
    else:
        await update.message.reply_text(
            f"#{tag} added to blacklist (was not in any group)."
        )
...
async def add_hashtag(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2 or args[1].upper() not in ("A", "B"):
        await update.message.reply_text(
            "Usage: /addhashtag <tag> <A|B>  e.g. /addhashtag hustle A"
        )
        return
    ...
    if tag in config.get("blacklist", []):
        await update.message.reply_text(
            f"#{tag} is blacklisted. Remove from blacklist first."
        )
        return
...
def move_pipeline_card_by_username(username, from_stage, to_stage):
    pipeline_path = os.path.join(VAULT, "03_CRM/Pipeline.md")
...
def log_call_outcome(username, outcome):
    calls_path = os.path.join(VAULT, "services/calls_log.json")
    try:
        with open(calls_path) as f:
            data = json.load(f)
    except Exception:
        data = {"calls": [], "showed": 0, "noshow": 0, "show_rate": 0.0}
    ...
    data["showed"] = sum(1 for c in data["calls"] if c["outcome"] == "showed")
    data["noshow"] = sum(1 for c in data["calls"] if c["outcome"] == "noshow")
    total = data["showed"] + data["noshow"]
    if total > 0:
        data["show_rate"] = round(data["showed"] / total * 100, 1)
...
async def closed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        revenue = {"clients": [], "total": 0.0, "monthly": {}}
    ...
    move_pipeline_card_by_username(username, "Booked", "Won")
...
async def revenue(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    recent_text = "\n".join(
        [
            f"  @{c['username']} — ${c['amount']:.0f} ({c['date']})"
...
async def showed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if not args:
        await update.message.reply_text("Usage: /showed username")
        return
    username = args[0].lstrip("@")
...
async def noshow(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if not args:
        await update.message.reply_text("Usage: /noshow username")
        return
    username = args[0].lstrip("@")
    log_call_outcome(username, "noshow")
    move_pipeline_card_by_username(username, "Booked", "Lost")
    await update.message.reply_text(
...
async def gateway_research(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = (
        " ".join(context.args).strip() if context.args else "analyze recent ICP signals"
    )
...
async def capture_context(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from umh.runtime_engine.gateway import ingest_external_context
...
async def decide(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.strategy_engine import DecisionEngine
...
async def portfolio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.portfolio_advisor import PortfolioAdvisor
...
async def board(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not question:
        await update.message.reply_text(
            "Usage: /board <question>  e.g. /board where should I focus this week?"
        )
        return
...
async def truth(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    venture_id = (
        (context.args[0] if context.args else "lyfe_institute")
        .lower()
        .replace(" ", "_")
    )
...
async def research_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.research_engine import ResearchEngine
...
async def gaps(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if gap_list:
            ...
            text = (
                "No knowledge gaps detected. Run /research to build knowledge manually."
            )
        if len(text) > 4000:
            text = text[:3990] + "\n...[truncated]"
    except Exception as e:
...
async def domains_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        from umh.runtime_engine.knowledge_domains import KnowledgeDomainRegistry
...
async def domain_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    key = (
        " ".join(context.args).strip().lower().replace(" ", "_") if context.args else ""
    )
    if not key:
        await update.message.reply_text(
            "Usage: /domain <key>\n"
...
async def trinity_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        sys.path.insert(0, "/opt/OS")
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.os_trinity import OSTrinity
...
async def connect_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        sys.path.insert(0, "/opt/OS")
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.os_trinity import OSTrinity, VALID_PRODUCTS
        ...
        if product not in VALID_PRODUCTS:
            ...
            return
        trinity = OSTrinity(ctx)
        ok = trinity.register_product(
            ctx.user_id, product, {"registered_via": "telegram"}
        )
...
async def permit_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text = (
            ...
            if ok
            else f"Failed to grant permission"
        )
    except Exception as e:
...
async def revoke_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text = (
            ...
            if ok
            else f"Failed to revoke permission"
        )
    except Exception as e:
...
async def handle_outcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if outcome_type not in _DEFAULT_SCORES:
        await update.message.reply_text(
            f"Unknown type: {outcome_type}\nValid: {', '.join(_DEFAULT_SCORES.keys())}"
        )
        return
...
async def evolve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Running evolution cycle — this may take a minute..."
    )
    try:
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.evolution_engine import EvolutionEngine
...
async def journey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            "Usage: /journey <username>  e.g. /journey johndoe"
        )
        return
    ...
    try:
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.knowledge_graph import KnowledgeGraph
...
async def meeting_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ctx = load_context_from_env()
    vi = _get_vi(ctx)
    _meeting_session_id = vi.start_meeting_session(f"{meeting_type}: {attendee}")
...
async def handle_sync(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from umh.runtime_engine.skill_registry import SkillRegistry
...
async def handle_media_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.voice:
        ...
        from umh.runtime_engine.media_processor import MediaProcessor

        modality = MediaProcessor().detect_modality(f"file{suffix}")
        if modality == "unknown":
            lock.release()
            await msg.reply_text(f"File type {suffix} not supported yet.")
            return
    ...
    await msg.reply_text(status_map.get(modality, "⏳ Processing..."))
    ...
    try:
        if modality in ("voice", "audio"):
            ...
            # Non-voice media — resolve to text via MediaProcessor, then spine
            from umh.runtime_engine.media_processor import MediaProcessor
            ...
            if not text:
                text = caption or f"[{modality} file uploaded, no text extracted]"

            _sid = str(_uuid.uuid4())
            _uctx = ContextBuilder().build(
                ctx,
                text,
                _sid,
            ...
            reply = output or "Could not process this media"
        ...
        if intent == "BRIEF":
            ...
            for vid, tiers in summary.items():
                if isinstance(tiers, dict) and "error" not in tiers:
                    ...
                    h = tiers.get("HIGH", 0)
                    if c + h > 0:
                        lines.append(f"{vid}: {c} critical, {h} high")
...
async def executions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from umh.runtime_engine.execution_engine import ExecutionEngine
        from umh.environments.system_context import load_context_from_env
...
async def trace_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not task_id:
            await update.message.reply_text(
                f"No task found with ID prefix: {task_prefix}"
            )
            return
...
async def pulse_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        from umh.environments.system_context import load_context_from_env
        from umh.runtime_engine.world_pulse import WorldPulse
...
async def _run_pre_meeting_automation(
    ...
    call_guidance = ""
    try:
        from umh.runtime_engine.agent_teams import run_team_task
...
async def standup_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        queue = ce.get_task_queue()
        today_active = [
            t for t in queue if t.get("status") in ("in_progress", "pending")
        ]
        ...
        unified_ctx = ContextBuilder().build(
            ctx,
            standup_prompt,
            session_id,
            agent="ceo_agent",
...
async def handle_browser_command(
    ...
    try:
        from umh.runtime_engine.browser_agent import run_browser_task

        result = await run_browser_task(url, task)
        steps = "\n".join(f"  \u2022 {s}" for s in result.get("steps_taken", []))
        reply = (
...
try:
    ...
    from umh.environments.system_context import load_context_from_env as _lcfe

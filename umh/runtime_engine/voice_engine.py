...
class VoiceEngine:
    ...
    def query_local(self, prompt: str, system: str | None = None) -> str:
        """
        Query local LLM via model_router.call_with_fallback().

        Routes through the canonical model_router path instead of direct
        Ollama HTTP. Falls back gracefully if no provider is available.
        ...
        try:
            ...
            result = call_with_fallback(
                ...
                trigger_source="voice_engine",
            )
            response = result.output or ''
            if response:
                print(f'[VoiceEngine] model_router: {response[:80]!r}')
            return response
        except Exception as e:
            print(f'[VoiceEngine] model_router query failed: {e}')
        return ''

...
Usage::
    ...
    from umh.runtime_engine.goal_mode import GoalMode, resolve_mode, infer_mode
...
_INFERENCE_RULES: list[tuple[re.Pattern, GoalMode]] = [
    ...
    (
        re.compile(
            r"\b(quick|fast|brief|tldr|tl;dr|one[- ]?liner|short answer)\b", re.I
        ),
        GoalMode.FAST,
    ),
...
def resolve_mode(
    ...
    if explicit is not None:
        try:
            ...
            _log.debug(
                "Invalid explicit goal_mode '%s', falling back to inference", explicit
            )

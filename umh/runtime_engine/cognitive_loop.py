"""
CognitiveLoop — DEPRECATED (2026-04-21).

Superseded by :class:`umh.execution_spine.ExecutionSpine` which is
now the single canonical runtime for all LLM operations in EOS.

This module is retained only for:
    - ``CognitiveResult`` / ``MultimodalInput`` — referenced by the
      legacy CognitiveLoop class.
    - ``format_response_footer`` — re-exported from execution_spine
      for backward compatibility with any external import paths.

``format_response_footer()`` and ``_get_neon_spend()`` now live in
``umh.execution_spine``.  The re-export here prevents ImportError
for any code that still uses the old import path.

Do NOT instantiate ``CognitiveLoop`` in new code.  All new LLM calls
must go through ``ExecutionSpine().run()``.
"""
...
class CognitiveLoop:
    """DEPRECATED — use :class:`umh.execution_spine.ExecutionSpine` instead.

    Retained as a reference artifact. All production paths now route
    through ExecutionSpine. Instantiating this class emits a
    DeprecationWarning.
    """
    ...
    def __init__(self, ctx: EOSContext):
        ...
        warnings.warn(
            "CognitiveLoop is deprecated — use ExecutionSpine instead. "
            "See umh/execution_spine.py for the canonical runtime.",
            DeprecationWarning,
            stacklevel=2,
        )
        self.ctx = ctx
...
def detect_intent_and_inject(
    ...
    if any(
        ...
        try:
            from umh.runtime_engine.meetings import draft_meeting_minutes  # noqa: F401

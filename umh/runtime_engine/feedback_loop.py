...
class FeedbackLoop:
    ...
    def log_recommendation(
        ...
        When ``evaluation`` is provided (from outcome_evaluator), quality
        signals are stored alongside the recommendation for learning.
        """
    ...
    def _classify_outcome_semantic(self, text: str) -> str:
        ...
        """
        try:
            from umh.execution.engine import lightweight_execute, LightweightTaskType
            ...
            result = lightweight_execute(
                ...
                task_type=LightweightTaskType.CLASSIFY_INTENT,
            )
            output = (result.outputs.get("response", "") or "").upper().strip()

...
Usage:
    ...
    from umh.runtime_engine.strategy_engine import StrategyEngine, DecisionEngine
...
def _query_30d_stats(org_id: str, venture_id: str) -> dict:
    ...
    return {
        ...
        "last_activity": str(row["last_activity"])[:19]
...
class StrategyEngine:
    ...
    def analyze_company_position(self, org_id: str) -> dict:
        ...
        for vid in venture_ids:
            try:
                ...
                pct = (
                    ...
                    if v.monthly_target and v.monthly_target > 0
                    else 0.0
                )
                block = (
                    f"{ctx_text}\n\n"
        ...
        _uctx = ContextBuilder().build(
            self.ctx, prompt, _sid, agent="strategy_engine.position"
        )
        output = run_via_umh(
            message=prompt,
            unified_context=_uctx,
            agent_type="strategy_engine.position",
        ...
        parsed = _parse_labeled_sections(output or "", keys)
    ...
    def analyze_portfolio_strategy(self) -> dict:
        ...
        _sid = str(_uuid.uuid4())
        _uctx = ContextBuilder().build(
            self.ctx, prompt, _sid, agent="strategy_engine.portfolio"
        )
        output = run_via_umh(
            message=prompt,
            unified_context=_uctx,
            agent_type="strategy_engine.portfolio",
            authority_class="analyze",
            session_id=_sid,
            org_id=str(self.ctx.org_id),
            task_type=TaskType.ANALYZE,
    ...
    def run_decision_analysis(
        ...
        output = run_via_umh(
            message=prompt,
            unified_context=_uctx,
            agent_type="strategy_engine.decision",
            authority_class="analyze",
            session_id=_sid,
            org_id=str(self.ctx.org_id),
            task_type=TaskType.ANALYZE,
...
class DecisionEngine:
    ...
    def evaluate(
        ...
        base = f"Decision under evaluation: {decision}\nContext: {context_str}\n\n"
        ...
        for step_key, step_prompt in steps:
            try:
                ...
                step_output = run_via_umh(
                    message=step_prompt,
                    unified_context=_uctx,
                    agent_type=f"decision_engine.{step_key}",
                    authority_class="analyze",
                    session_id=_sid,
                    org_id=str(self.ctx.org_id),
                    task_type=TaskType.ANALYZE,
                ...
                results[step_key] = step_output or ""
            except Exception as e:

...
class CEOAgent:
    ...
    def detect_primitives(self) -> dict:
        ...
        try:
            ...
            with get_conn(self.ctx.org_id) as cur:
                ...
                outcomes = {r["outcome"]: r["count"] for r in cur.fetchall() if r["outcome"]}
    ...
    def reason_org_chart(
        ...
        try:
            ...
            result = utility_llm_call(prompt, operation="ceo_agent_analysis")

            roles = [r.strip() for r in result.split(",") if r.strip() in available]
    ...
    def get_active_constraint(
        ...
        try:
            ...
            vid = venture_id or self.ctx.active_venture_id or "lyfe_institute"
            return diagnose_constraint(vid, self.ctx)
        except Exception as e:
            print(f"[CEOAgent] constraint: {e}")
            return {
                "constraint": "leads",
                "recommendation": "Increase outreach volume.",
                "active_agents": [
                    "research_agent",
                    "outreach_agent",
                ],

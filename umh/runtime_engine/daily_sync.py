...
class DailySync:
    ...
    def build_agenda(self) -> SyncAgenda:
        ...
        try:
            from umh.runtime_engine.meetings import get_open_loop_meetings
        ...
        if len(agenda.action_items) > 1:
            try:
                ...
                items_text = "\n".join(
                    f"{i + 1}. {item}" for i, item in enumerate(agenda.action_items)
                )
                prompt = (
                ...
                result = utility_llm_call(prompt, operation="daily_sync_prioritize").strip()
                if "```" in result:
        ...
        try:
            ...
            _constraint = (
                _binding.recommendation if _binding else "Close the first Initiate Arena client"
            )
            ...
            if _top_item and _top_item != "No open action items.":
                ...
                agenda.goal_alignment = utility_llm_call(
                    _align_prompt, operation="daily_sync_alignment"
                ).strip()
        except Exception:
...
{{
  ...
  "recurring_3": ["Task DEX owns completely every day", "Task 2", "Task 3"]
            ...
            _333_result = utility_llm_call(_333_prompt, operation="daily_sync_333").strip()
            if "```" in _333_result:
        ...
        try:
            from umh.runtime_engine.task_yield_matrix import classify_task_yield
        ...
        try:
            ...
            try:
                ...
                if sla_breaches:
                    agenda.emails.insert(
                        0, f"⚠️ **SLA breach — {len(sla_breaches)} emails over 24h:**"
                    )
        ...
        try:
            ...
            for r in rows:
                payload = (
                    r["payload_json"]
                    if isinstance(r["payload_json"], dict)
                    else _json.loads(r["payload_json"])
                )
        ...
        try:
            from umh.runtime_engine.subscription_tracker import get_upcoming_renewals
        ...
        try:
            ...
            if upcoming:
                agenda.important_dates = [
                    f"{d['person']} — {d['type']} in {d['days_until']} days" for d in upcoming[:3]
                ]
            else:
                agenda.important_dates = []
        except Exception:
    ...
    def format_sync_message(self, agenda: SyncAgenda, closing_line: str = "") -> str:
        ...
        if agenda.quarterly_rocks:
            ...
            lines.append(f"_🪨 {_q} Rocks: {' | '.join(agenda.quarterly_rocks[:3])}_")
        if agenda.dex_items:
            lines.append(f"_🤖 DEX handling ({len(agenda.dex_items)} below BBR):_")

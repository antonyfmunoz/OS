...
from umh.runtime_engine.strategy_engine import _parse_labeled_sections
...
class ResearchEngine:
    ...
    def detect_knowledge_gaps(self) -> list[str]:
        ...
        if not interactions:
            print(
                "[ResearchEngine] No interaction history found — using foundational gaps"
            )
        ...
        _uctx = ContextBuilder().build(
            self.ctx, prompt, _sid, agent="research_engine.gap_detector"
        )
        output = run_via_umh(
            message=prompt,
            unified_context=_uctx,
            agent_type="research_engine.gap_detector",
        ...
        print(
            f"[ResearchEngine] {len(gaps)} knowledge gaps detected from interaction history"
        )
        return gaps
    ...
    def _detect_foundational_gaps(self) -> list[str]:
        ...
        _sid = str(_uuid.uuid4())
        _uctx = ContextBuilder().build(
            self.ctx, prompt, _sid, agent="research_engine.foundational_gaps"
        )
        output = run_via_umh(
            message=prompt,
            unified_context=_uctx,
            agent_type="research_engine.foundational_gaps",
            authority_class="analyze",
            session_id=_sid,
            org_id=str(self.ctx.org_id),
            task_type=TaskType.ANALYZE,
    ...
    def research_topic(
        ...
        """
        from umh.runtime_engine.scrapling_connector import ScraplingConnector
        ...
        if _scraped_pages:
            try:
                ...
                print(
                    f"[ResearchEngine] KnowledgeIntegrator failed (non-blocking): {_ki_e}"
                )
        ...
        output = run_via_umh(
            message=prompt,
            unified_context=_uctx,
            agent_type="research_engine.researcher",
    ...
    def store_knowledge(
        ...
        if not ko_content or len(ko_content) < 100:
            print(
                f"[ResearchEngine] store_knowledge skipped — content too thin: '{topic[:60]}'"
            )
            return False
    ...
    def scan_ai_landscape(self) -> dict:
        ...
        """
        import os as _os
        from umh.runtime_engine.model_preferences import PROVIDER_CONFIGS
        ...
        _sid = str(_uuid.uuid4())
        _uctx = ContextBuilder().build(
            self.ctx, prompt, _sid, agent="research_engine.ai_landscape_scanner"
        )
        output = run_via_umh(
            message=prompt,
            unified_context=_uctx,
            agent_type="research_engine.ai_landscape_scanner",
            authority_class="analyze",
            session_id=_sid,
            org_id=str(self.ctx.org_id),
            task_type=TaskType.ANALYZE,
        ...
        updated_costs = self._parse_model_costs(output or "")
        if updated_costs:
            import umh.runtime_engine.agent_runtime as _ar
    ...
    def _parse_model_costs(self, scan_text: str) -> dict:
        ...
        _sid = str(_uuid.uuid4())
        _uctx = ContextBuilder().build(
            self.ctx, _extract_prompt, _sid, agent="research_engine.cost_extractor"
        )
        extract_output = run_via_umh(
            message=_extract_prompt,
            unified_context=_uctx,
            agent_type="research_engine.cost_extractor",
            authority_class="analyze",
            session_id=_sid,
            org_id=str(self.ctx.org_id),
    ...
    def run_domain_update_cycle(self) -> dict:
        ...
        for domain_key in due_domains:
            ...
            try:
                ...
                raw = (
                    run_via_umh(
                        message=_score_prompt,
                        unified_context=_uctx,
                        agent_type="research_engine.domain_scanner",
                        authority_class="analyze",
                        session_id=_sid,
                        org_id=str(self.ctx.org_id),
                        task_type=TaskType.CLASSIFY,
        ...
        for domain_key, score in sorted_domains:
            if score < 6:
                print(
                    f"[ResearchEngine] Domain {domain_key} score={score} — skipping deep research"
                )
                continue
            domain = registry.get_domain(domain_key)
            if not domain:
            ...
            try:
                ...
                deep_output = run_via_umh(
                    message=_deep_prompt,
                    unified_context=_uctx,
                    agent_type="research_engine.domain_researcher",
                    authority_class="analyze",
                    session_id=_sid,
                    org_id=str(self.ctx.org_id),
                    task_type=TaskType.ANALYZE,
        ...
        print(
            f"[ResearchEngine] ── Domain update cycle complete: "

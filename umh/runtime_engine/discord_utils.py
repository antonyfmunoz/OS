...
def post_to_webhook(
    ...
    try:
        from umh.runtime_engine.output_validator import OutputValidator
    ...
    for i, chunk in enumerate(chunks):
        try:
            ...
            if resp.status_code not in [200, 204]:
                print(
                    f"[Discord] Chunk {i + 1}/{len(chunks)} status: {resp.status_code}"
                )
                success = False
            time.sleep(0.5)
        except Exception as e:
...
async def post_to_channel(
    ...
    """
    Post content to a Discord channel object with delivery guarantees.

    Routes through the unified send_reply() pipeline which provides
    structure-aware chunking, attachment fallback, and delivery tracking.
    Returns True if all content delivered, False otherwise.
    """
    from umh.substrate.session_discord_bridge import send_reply

    text = f"**{title}**\n\n{content}" if title else content
    return await send_reply(channel, text)

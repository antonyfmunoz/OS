...
from umh.substrate.workload_policy import classify_workload
from umh.substrate.resource_guard import evaluate_resource_guard
from umh.substrate.context_lifecycle import (
...
def ingest_text_message(
    ...
    *,
    ...
    try:
        from umh.substrate.workflow_delegation import enrich_metadata
    ...
    if (
        ...
        try:
            from umh.substrate.workflow_execution import (
    ...
    try:
        with mode_context(
            ...
            _spine_result = run_via_umh(
                message=clean,
                unified_context=meta.get("unified_context", {}),
                channel_id=cid or "unknown",
                user_id=uid or "unknown",
                venture_id=meta.get("venture_id"),
            )
            ec_result = {
                "response": str(_spine_result),
                "ok": bool(_spine_result),
                "session_id": getattr(_spine_result, "session_id", None),
                "provider": getattr(_spine_result, "provider", ""),
                "path": "spine",
                "error": None if _spine_result else "spine returned empty",
            }
    except Exception as e:  # noqa: BLE001
        ev = DiscordTextEvent(
            kind="error",
...
def build_tts_reply_envelope(
    ...
    *,
    ...
    try:
        from umh.substrate.tts_sanitize import sanitize_tts_reply
...
def _handle_session_command(
    ...
    *,
    ...
    try:
        from umh.substrate.session_control import clear_session, reset_session
...
def maybe_mirror_discord_text_message(
    ...
    *,
    ...
    # substrate → broader router (umh.model_router.call_with_fallback),
    ...
    try:
        from umh.substrate.discord_mode_routing import resolve_discord_mode
        from umh.substrate.mode_behavior import shape_reply
    ...
    try:
        from umh.substrate.discord_mode_routing import resolve_discord_mode as _rdm
        from umh.substrate.discord_mode_routing import resolve_mode_session as _rms
...
def _hybrid_execution_status() -> dict[str, Any]:
    ...
    try:
        from umh.substrate.target_policy import (
            POLICY_VERSION,

...
(see umh.substrate.capabilities) will consult the node registry to pick
a target, but that integration is intentionally deferred.

Usage:
    from umh.world.substrate import NodeRegistry, Node, NodeType, NodeStatus
...
class Node:
    ...
    this module has no import-cycle with umh.substrate.capabilities.
...
class NodeRegistry:
    ...
    umh.substrate.storage on every upsert/remove, so nodes survive across

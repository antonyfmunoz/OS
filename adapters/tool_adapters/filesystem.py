...
class FilesystemAdapter(BaseAdapter):
    ...
    _DENIED_OPERATIONS: frozenset[str] = frozenset(
        {"delete", "rm", "rmdir", "unlink", "chmod", "chown"}
    )
    ...
    def _is_safe_rooted(self, path: str) -> bool:
        ...
        try:
            ...
            return False
        return any(
            resolved.startswith(root + os.sep) or resolved == root for root in self._safe_roots
        )
    ...
    def _op_mkdir(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "path": str(path),
            "created": True,
            "_side_effects": [f"created directory {path_str}"],
        }

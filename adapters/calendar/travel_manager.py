...
def detect_travel_event(event: dict) -> bool:
    ...
    return (
        any(s in title for s in travel_signals)
        or any(s in location for s in location_signals)
        or any(s in description for s in travel_signals)
    )
...
def build_travel_brief(
    ...
    try:
        ...
        _ctx = ctx or _lctx()
        _traveler = (
            getattr(_ctx, "founder_name", None) or getattr(_ctx, "user_name", None) or "the founder"
        )
...
Note: Research only — Antony makes the final booking.""",
        ).strip()
    except Exception as e:
...
Be specific with real restaurant names.""",
        ).strip()
    except Exception as e:
...
def generate_trip_itinerary(
    ...
    try:
        ...
        meetings_text = (
            "\n".join(f"- {m}" for m in (meetings or []))
            if meetings
            else "No meetings confirmed yet"
        )

        itinerary = router.call_with_fallback(
            TaskType.ANALYSIS,
            f"""Generate a detailed trip itinerary.

            ...
            resp = requests.post(
                ...
                json={
                    "filter": {
                        ...
                        "rich_text": {"equals": calendly_event_id},
                    }
                },
                timeout=10,
            )
...
{{"stage": "New Lead|Contacted|Qualified|Proposal|Negotiation|Closed Won|Closed Lost|Nurture",
  ...
  "should_update": true/false}}"""
                    ...
                    if _stage_data.get("should_update") and _stage_data.get("confidence") in (
                        "high",
                        "medium",
                    ):
            ...
            meetings.append(
                {
                    ...
                    .get("rich_text", [{}])[0]
                    .get("plain_text", ""),
                    "open_loops": props.get("Open Loops", {})
                    .get("rich_text", [{}])[0]
                    .get("plain_text", ""),
...
def draft_meeting_agenda(
    ...
    try:
        ...
        profile = build_intelligence_profile(name=person, email=email, ctx=ctx)
...
def draft_meeting_minutes(
    ...
    try:
        ...
        minutes = router.call(
            model,
            f"""Draft formal meeting minutes.
...
def calculate_meeting_roi(
    ...
    try:
        ...
        if venture:
            filter_clause["and"].append({"property": "Venture", "select": {"equals": venture}})
        ...
        for m in meetings:
            ...
            if meeting_type not in stats["by_type"]:
                stats["by_type"][meeting_type] = {"total": 0, "completed": 0, "advanced": 0}
            stats["by_type"][meeting_type]["total"] += 1
            if status == "Completed":
                stats["by_type"][meeting_type]["completed"] += 1
                if any(
                    kw in outcomes.lower()
                    for kw in [
                        "closed",
                        "proposal",
                        "next step",
        ...
        if stats["total"] > 0:
            stats["conversion_rate"] = round(stats["completed"] / stats["total"], 2)

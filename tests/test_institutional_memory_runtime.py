...
class TestNoDeps:
    ...
    def setup_class(cls) -> None:
        cls.rt = InstitutionalMemoryRuntime()

    def test_propose_creates_proposed(self) -> None:
        k = self.rt.propose("test knowledge", "test", "src-1")
        assert k.state == KnowledgeState.PROPOSED.value
...
class TestKnowledgeLifecycle:
    ...
    def test_propose(self) -> None:
        k = self.rt.propose("lesson A", "lesson", "l-1")
        assert k.state == "proposed"
        assert k.validations == 0
        assert k.confidence == 0.3

    def test_validate_increments(self) -> None:
        k = self.rt.propose("lesson B", "lesson", "l-2")
        result = self.rt.validate(k.knowledge_id)
        assert result is not None
        assert result.validations == 1
        assert result.state == "proposed"

    def test_validate_promotes_to_validated(self) -> None:
        k = self.rt.propose("lesson C", "lesson", "l-3")
        self.rt.validate(k.knowledge_id)
        result = self.rt.validate(k.knowledge_id)
        assert result is not None
    ...
    def test_promote_to_canonical(self) -> None:
        k = self.rt.propose("lesson D", "lesson", "l-4")
        self.rt.validate(k.knowledge_id)
        self.rt.validate(k.knowledge_id)
        self.rt.validate(k.knowledge_id)
        k.confidence = 0.8
        result = self.rt.promote(k.knowledge_id)
        assert result is not None
    ...
    def test_promote_fails_without_threshold(self) -> None:
        k = self.rt.propose("lesson E", "lesson", "l-5")
        result = self.rt.promote(k.knowledge_id)
        assert result is not None
        assert result.state == KnowledgeState.PROPOSED.value

    def test_supersede(self) -> None:
        k = self.rt.propose("old knowledge", "memory", "m-1")
        result = self.rt.supersede(k.knowledge_id, "replacement-id")
        assert result is not None
        assert result.state == KnowledgeState.SUPERSEDED.value

    def test_retire(self) -> None:
        k = self.rt.propose("retiring knowledge", "memory", "m-2")
        result = self.rt.retire(k.knowledge_id)
        assert result is not None
        assert result.state == KnowledgeState.RETIRED.value

    def test_validate_nonexistent_returns_none(self) -> None:
        result = self.rt.validate("nonexistent-id")
...
class TestFiltering:
    ...
    def test_knowledge_by_state_all(self) -> None:
        baseline = len(self.rt.knowledge_by_state())
        self.rt.propose("a", "test", "1")
        self.rt.propose("b", "test", "2")
        all_k = self.rt.knowledge_by_state()
        assert len(all_k) == baseline + 2

    def test_knowledge_by_state_filter(self) -> None:
        baseline_proposed = len(self.rt.knowledge_by_state("proposed"))
        k1 = self.rt.propose("a", "test", "1")
        k2 = self.rt.propose("b", "test", "2")
        self.rt.validate(k1.knowledge_id)
        self.rt.validate(k1.knowledge_id)
        proposed = self.rt.knowledge_by_state("proposed")
        validated = self.rt.knowledge_by_state("validated")
        assert len(proposed) == baseline_proposed + 1
        assert len(validated) == 1

    def test_canonical_knowledge(self) -> None:
        baseline_canonical = len(self.rt.canonical_knowledge())
        k = self.rt.propose("canonical", "test", "1")
        self.rt.validate(k.knowledge_id)
        self.rt.validate(k.knowledge_id)
        self.rt.validate(k.knowledge_id)
        k.confidence = 0.8
        self.rt.promote(k.knowledge_id)
        canonical = self.rt.canonical_knowledge()
        assert len(canonical) == baseline_canonical + 1
        assert canonical[-1].state == "canonical"

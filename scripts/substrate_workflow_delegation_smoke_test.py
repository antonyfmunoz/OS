...
def test_builder_dev_classification() -> None:
    ...
    from runtime.transport.workflow_delegation import classify_workflow_intent
...
def test_no_hotpath_imports() -> None:
    ...
    src = importlib.util.find_spec("runtime.transport.workflow_delegation")
...
def test_transport_integration_returns_workflow_fields() -> None:
    ...
    src = importlib.util.find_spec("runtime.transport.discord_text_transport")
    assert src and src.origin
    with open(src.origin) as f:
        content = f.read()

...
4. **Register the role/permission canonicals** already identified: `PermissionTier`/`AutonomyLevel` were already registered; `AgentRole`/`RoleScope`/`RoleRegistry` were NOT — register them (they are real symbols in `substrate.execution.bridge.roles`).
...
- Two HARD-GATE *risk* fail-open sites fixed + tested:
...
**Scope of this packet — risk vs. role/permission (explicit, to prevent future overclaim):**
WP-P2-002 **fully hardens the canonical RISK vocabulary** (declares the unambiguous two-axis spine, adds the one fail-closed coercion helper, closes the two hard risk gates) and **verifies + registers the existing ROLE/PERMISSION canonicals**: `PermissionTier`/`AutonomyLevel` were already registered; `AgentRole`/`RoleScope`/`RoleRegistry` existed as real symbols in `substrate.execution.bridge.roles` but were **NOT registered** — this packet registers them (verified via the hardened #152 registry audit) so the divergence gate now polices them. No role type was rebuilt or created. It does **NOT** fully converge every role/permission-envelope behavior: the role and permission canonicals already existed and were not rebuilt, and no unified role/permission runtime was introduced (out of mandate — "no new role/permission system").

**Known permission-envelope fail-open — tracked follow-on, NOT fixed here:**
`required_tier_for_action(action_type)` returns `READ` (the least-privileged tier) for an unknown/unmapped action — a fail-open, since it is consumed as the *minimum tier a caller must hold* by three authority gates (`authority_engine:108`, `execution_authority_engine_v1:435`, `control_plane/governance:128`). The correct fail-closed default is `COMMIT`. This is **deliberately deferred**, not silently accepted: a blanket unknown→COMMIT change was implemented and reverted because the authority engine couples COMMIT-tier to CRITICAL/DENY risk amplification, so it wrongly escalates deliberately-HIGH actions (e.g. `data_deletion`, expected HIGH/APPROVE_EXECUTE) to CRITICAL/DENY, breaking 5 authority-engine tests. Converging it requires reworking the engine's risk classification — out of this HIGH-risk packet's mandate ("no new risk taxonomy"). Filed as a follow-on packet.

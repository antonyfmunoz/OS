...
async def on_message(message: discord.Message):
    ...
    if pipeline_update:
        ...
        try:
            ...
            _sys.path.insert(0, "/opt/OS/services")
    ...
    if text.startswith("!nomeetings"):
        ...
        try:
            ...
            if len(_nm_parts) > 1:
                ...
                try:
                    _nm_block_date = datetime.fromisoformat(_nm_date_str).replace(
                        tzinfo=_nm_PDT
                    )
                except Exception:
    ...
    if text.startswith("!meetingroi"):
        ...
        try:
            ...
            if _roi["top_converting_type"]:
                _roi_lines.append(
                    f"Best converting type: {_roi['top_converting_type']}"
                )
            if _roi.get("by_type"):
                _roi_lines.append("\n**By type:**")
                for _mtype, _mdata in _roi["by_type"].items():
                    _mrate = (
                        _mdata["advanced"] / _mdata["total"]
                        if _mdata["total"] > 0
                        else 0
                    )
                    _roi_lines.append(
    ...
    if any(kw in text.lower() for kw in _cal_keywords):
        try:
            ...
            _gws = GWSConnector()
            from eos_ai.model_router import call_with_fallback as _cal_route
            ...
            _cal_result = _cal_route(prompt=_cal_prompt, task_type="fast_response")
            import json as _json

            _parsed = _json.loads(_cal_result.output.strip())

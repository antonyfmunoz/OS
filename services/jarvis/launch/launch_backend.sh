#!/usr/bin/env bash
# Launch Jarvis backend API on port 8093.
#
# Usage:
...
echo "Starting Jarvis backend on ${HOST}:${PORT}..."

if [[ "${1:-}" == "--bg" ]]; then
    nohup python3 -m uvicorn umh_mvp.api.app:app \
        --host "$HOST" --port "$PORT" --reload \
        > /opt/OS/logs/jarvis_backend.log 2>&1 &
    echo "PID: $!"
    echo "Log: /opt/OS/logs/jarvis_backend.log"
else

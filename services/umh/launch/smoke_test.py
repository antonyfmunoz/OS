#!/usr/bin/env python3
"""UMH smoke test — verifies all integration points are operational.
...
def test_backend_health() -> SmokeTestResult:
    ...
    try:
        code, data = _get(f"{BACKEND}/api/umh/health")
...
def test_trace_endpoint() -> SmokeTestResult:
    ...
    start = time.monotonic()
    try:
        code, data = _get(f"{BACKEND}/api/umh/traces?limit=5")
        r.duration_ms = int((time.monotonic() - start) * 1000)
        r.data = data
...
def test_resume_endpoint() -> SmokeTestResult:
    ...
    start = time.monotonic()
    try:
        code, data = _get(f"{BACKEND}/api/umh/resume")
        r.duration_ms = int((time.monotonic() - start) * 1000)
        r.data = data
...
def test_awareness_endpoint() -> SmokeTestResult:
    ...
    start = time.monotonic()
    try:
        code, data = _get(f"{BACKEND}/api/umh/awareness")
        r.duration_ms = int((time.monotonic() - start) * 1000)
        r.data = data
...
def test_capabilities_endpoint() -> SmokeTestResult:
    ...
    start = time.monotonic()
    try:
        code, data = _get(f"{BACKEND}/api/umh/capabilities")
        r.duration_ms = int((time.monotonic() - start) * 1000)
        r.data = data
...
def test_routing_config() -> SmokeTestResult:
    ...
    try:
        ...
        from services.umh.model_routing.capabilities import (
            CapabilityClass,
            CAPABILITY_REGISTRY,
        )

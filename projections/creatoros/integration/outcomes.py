"""CreatorOS outcome receiver — writes pipeline outcomes back to CreatorOS Postgres.
...
class CreatorOSOutcomeReceiver:
    """Receives pipeline outcomes and writes them back to CreatorOS Postgres.
    ...
    def __init__(
        self,
        database_url: str,
        correlation_map: CreatorOSCorrelationMap,
    ) -> None:
    ...
    def on_outcome(self, envelope: OutcomeEnvelope) -> None:
        if envelope.correlation_id is None:
            logger.debug("creatoros outcome: no correlation_id, skipping writeback")
            return

        target = self._correlation_map.lookup(envelope.correlation_id)
        if target is None:
            logger.debug(
                "creatoros outcome: correlation_id %s not in map",
                envelope.correlation_id,
            )
            return

        if target.integration != "creatoros":
            logger.debug(
                "creatoros outcome: target integration is %s, not creatoros",
                target.integration,
            )
            return
        ...
        try:
            ...
            logger.error(
                "creatoros writeback db error: %s for %s.%s",
                exc,
                target.table_name,
                target.row_id,
            )
            ...
            logger.error(
                "creatoros writeback failed: %s: %s for %s.%s",
                type(exc).__name__,
                exc,
                target.table_name,
                target.row_id,
    ...
    def _writeback(self, conn: Any, target: Any, envelope: OutcomeEnvelope) -> None:
        ...
        if mapped_status in SOURCE_ROW_UPDATE_TYPES and target.row_id:
            try:
                ...
                if updated:
                    logger.info(
                        "creatoros writeback: %s.%s umh_status=%s",
                        target.table_name,
                        target.row_id,
                        mapped_status,
                    )
                ...
                logger.error(
                    "creatoros writeback source row update failed: %s for %s.%s",
                    exc,
                    target.table_name,
                    target.row_id,
                )
        ...
        try:
            ...
            logger.info(
                "creatoros writeback: umh_outcomes id=%s type=%s trace=%s",
                audit_id,
                mapped_status,
                envelope.trace_id,
            )
            ...
            logger.error(
                "creatoros writeback audit insert failed: %s for trace %s",
                exc,
                envelope.trace_id,
            )

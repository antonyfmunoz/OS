...
def check_sandbox_edit_does_not_touch_production() -> None:
    ...
    prod_path = _REPO_ROOT / "eos" / "memory.py"
...
def check_sandbox_write_blocked_if_target_outside_workspace() -> None:
    ...
    try:
        ...
        prod_target = _REPO_ROOT / "eos" / "memory.py"
...
def check_cleanup_refuses_random_directories() -> None:
    ...
    env = Environment(
        ...
        name="evil",
        root=_REPO_ROOT / "eos",  # production path!
        workspace=_REPO_ROOT / "eos",
        data_dir=_REPO_ROOT / "eos",
        log_dir=_REPO_ROOT / "eos",
        state_dir=_REPO_ROOT / "eos",
        snapshot_dir=_REPO_ROOT / "eos",
    )
    try:
        env.cleanup()
    except PermissionError:
...
def check_graph_refresh_disabled_in_sandbox() -> None:
    ...
    try:
        a = ActionSystem(env=env)
        res = a._refresh_graph(["eos/memory.py"])
        _assert(

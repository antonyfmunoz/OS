...
def already_contacted(username):
    ...
    leads_dir = os.path.join(
        os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "03_CRM/Leads"
    )
...
def update_opener_stats_sent(opener_text):
    ...
    opener_stats_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "opener_stats.json"
    )
    try:
...
def push_lead_to_notion(
    ...
    payload = {
        ...
        "properties": {
            "Name": {"title": [{"text": {"content": f"@{username}"}}]},
            "Stage": {"select": {"name": "New Lead"}},
            "Channel": {"select": {"name": channel}},
            "Value": {"number": 750},
            "Last Contact": {"date": {"start": date.today().isoformat()}},
            "Notes": {"rich_text": [{"text": {"content": notes}}]},
            "Archetype": {"rich_text": [{"text": {"content": archetype}}]},
            "Score": {"number": score},
            "AI Qualified": {"checkbox": True},
        },
    }
...
def create_lead_file(
    username, comment_text, source, post_url, timestamp, result, opener, opener_index=0
):
...
PIPELINE_FILE = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "03_CRM/Pipeline.md"
)
...
def main():
    ...
    for filepath in unprocessed:
        if qualified >= MAX_LEADS_PER_RUN:
            print(
                f"Daily lead limit reached ({MAX_LEADS_PER_RUN}). Stopping to control costs."
            )
            break
        ...
        if not comment_text:
            ...
            shutil.move(
                filepath,
                os.path.join(PROCESSED_SIGNALS_DIR, os.path.basename(filepath)),
            )
            total += 1
            continue

        result, input_toks, output_toks = score_comment(
            runtime, comment_text, api_call_counter
        )
        ...
        if score >= 7 and not disqualify:
            if (
                lead_exists(username)
                or in_pipeline(username)
                or already_contacted(username)
            ):
                print(f"@{username} — DUPLICATE — already in system, skipping")
                shutil.move(
                    filepath,
                    os.path.join(PROCESSED_SIGNALS_DIR, os.path.basename(filepath)),
                )
            ...
            opener, opener_index = pick_opener(
                outreach_text, archetype, result["pain_signals"], comment_text
            )
            lead_filepath = create_lead_file(
                username,
                comment_text,
                source,
                post_url,
            ...
            add_to_kanban(
                ...
                archetype,
                comment_text,
                os.path.basename(lead_filepath).replace(".md", ""),
            )
            qualified += 1
        ...
        shutil.move(
            filepath, os.path.join(PROCESSED_SIGNALS_DIR, os.path.basename(filepath))
        )
        total += 1

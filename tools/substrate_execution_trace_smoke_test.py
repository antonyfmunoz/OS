...
def main() -> int:
    ...
    router_path = os.path.join("/opt/OS", "eos", "model_router.py")
    ...
    trace_path = os.path.join("/opt/OS", "eos", "substrate", "execution_trace.py")
    ...
    try:
        mod = importlib.import_module("umh.substrate.execution_trace")
    ...
    transport_path = os.path.join(
        "/opt/OS", "eos", "substrate", "discord_text_transport.py"
    )
    with open(transport_path) as f:
        transport_src = f.read()
    # Must not import cognitive_loop directly
    has_cognitive_import = "from eos.cognitive_loop" in transport_src
    # Must not import gateway directly
    has_gateway_import = "from eos.gateway" in transport_src
    check(

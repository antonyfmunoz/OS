...
function RoomHeader({ channel }: { channel: { name: string; topic: string; type: string; locked: boolean } }) {
  return (
    <div
      className="flex items-center px-4 h-9 shrink-0 border-b gap-2"
    ...
    >
      ...
      {channel.locked && (
        <span className="text-[9px] font-mono px-1.5 rounded" style={{ background: 'var(--color-warn-dim)', color: 'var(--color-warn)' }}>
          LOCKED
        </span>
      )}
      <span
        className="ml-auto text-[9px] font-mono uppercase px-1.5 rounded"

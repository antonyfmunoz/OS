...
const BuildLoopPanel = lazy(() =>
  import('../../panels/BuildLoopPanel').then((m) => ({ default: m.BuildLoopPanel })),
)
const OperatingLoopPanel = lazy(() =>
  import('../../panels/OperatingLoopPanel').then((m) => ({ default: m.OperatingLoopPanel })),
)
const OrganismLoopPanel = lazy(() =>
  import('../../panels/OrganismLoopPanel').then((m) => ({ default: m.OrganismLoopPanel })),
)
const TickLoopPanel = lazy(() =>
  import('../../panels/TickLoopPanel').then((m) => ({ default: m.TickLoopPanel })),
)
...
function StatePill({ state }: { state: PersistentLoopStatus['state'] }) {
  return (
    <span
      ...
      style={{ background: 'var(--color-surface-raised)', color: STATE_COLOR[state] }}
    >
      {state}
    </span>
  )
...
function CycleReport({
  ...
  cycle: NonNullable<PersistentLoopStatus['last_cycle']>
  title: string
}) {
  return (
    <div
    ...
    >
      <div className="text-[12px] font-medium mb-2" style={{ color: 'var(--color-text-primary)' }}>
        {title}
      </div>
      <div className="grid grid-cols-2 gap-y-1 text-[11px]" style={{ color: 'var(--color-text-secondary)' }}>
        <span>Started</span>
        <span style={{ color: 'var(--color-text-tertiary)' }}>{cycle.started_at}</span>
        <span>Finished</span>
        <span style={{ color: 'var(--color-text-tertiary)' }}>{cycle.finished_at}</span>
        <span>Actions taken</span>
        <span style={{ color: 'var(--color-text-tertiary)' }}>{cycle.actions_taken}</span>
        <span>Errors</span>
        <span style={{ color: cycle.errors > 0 ? '#ef4444' : 'var(--color-text-tertiary)' }}>{cycle.errors}</span>
      </div>
      {cycle.details.length > 0 && (
        <div className="mt-2 flex flex-col gap-1">
          {cycle.details.map((d, i) => (
            <pre
              key={i}
              className="text-[10px] p-1.5 rounded overflow-auto"
    ...
    <div className="absolute inset-0 flex items-start justify-center z-[5] pointer-events-none overflow-auto pt-12 pb-20 px-8">
      <div className="pointer-events-auto w-full max-w-[900px] flex flex-col gap-8">
        <section>
          ...
          {showCreate && (
            <div
              className="p-4 rounded-lg mb-4 flex flex-col gap-2"
              style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}
            >
              <input
                type="text"
                ...
                style={{ background: 'var(--color-surface-raised)', color: 'var(--color-text-primary)', border: '1px solid var(--color-border)', outline: 'none' }}
              />
              <input
                type="text"
                value={domain}
              ...
              <textarea
                ...
                rows={2}
                className="px-2 py-1 text-[12px] rounded resize-none"
                style={{ background: 'var(--color-surface-raised)', color: 'var(--color-text-primary)', border: '1px solid var(--color-border)', outline: 'none' }}
              />
          ...
          {loops.length === 0 ? (
            <div className="flex flex-col items-center gap-3 mt-8" style={{ color: 'var(--color-text-tertiary)' }}>
              <RefreshCcw size={32} />
              <span className="text-[13px]">No persistent loops</span>
              ...
              {loops.map((loop) => {
                ...
                return (
                  <div
                    ...
                    style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}
                    onClick={() => openLoop(loop.name, 'persistent')}
                  >
                    ...
                    <div className="text-[10px] mt-1.5" style={{ color: 'var(--color-text-tertiary)' }}>
                      {loop.interval_seconds}s interval · {loop.cycle_count} cycles
                    </div>
                    {loop.error_count > 0 && (
                      <div className="text-[10px] mt-1" style={{ color: '#ef4444' }}>
                        {loop.error_count} errors
                      </div>
                    )}
                    <div className="flex gap-1 mt-2" onClick={(e) => e.stopPropagation()}>
                      {!isRunning && (
                        <IconButton title="Start" color="#22c55e" onClick={() => startLoop(loop.name)}>
                          <Play size={12} />
                        </IconButton>
                      )}
                      {isRunning && (
                        <IconButton title="Stop" color="#ef4444" onClick={() => stopLoop(loop.name)}>
                          <Square size={12} />
                        </IconButton>
                      )}
                      <IconButton title="Run once" onClick={() => runOnce(loop.name)}>
                        <SkipForward size={12} />
                      </IconButton>
                      <IconButton title="Dry run" onClick={() => dryRun(loop.name)}>
        ...
        <section>
          <h2 className="text-[14px] font-medium mb-4" style={{ color: 'var(--color-text-primary)' }}>
            Lifecycle Loops
          </h2>
            ...
            {LIFECYCLE_CARDS.map((card) => {
              ...
              return (
                <div
                  ...
                  className="p-3 rounded-lg cursor-pointer"
                  style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}
                  onClick={() => openLoop(card.id, 'lifecycle')}
                >
                  <div className="flex items-center gap-2">
...
function PersistentLoopDetail({ loop }: { loop: PersistentLoopStatus }) {
  ...
  const toggleStage = useCallback((stage: string) => {
    setEditStages((prev) => (prev.includes(stage) ? prev.filter((s) => s !== stage) : [...prev, stage]))
  }, [])
  ...
  const handleSave = useCallback(async () => {
    ...
    setEditing(false)
  }, [loop.name, editInterval, editDescription, editDomain, editStages, updateLoop])

  const handleDelete = useCallback(async () => {
    if (!confirm(`Delete loop "${loop.name}"?`)) return
    await deleteLoop(loop.name)
    closeLoop()
  }, [loop.name, deleteLoop, closeLoop])
    ...
    <div className="flex flex-col gap-4 max-w-[900px] mx-auto w-full p-6 overflow-auto">
      ...
      >
        ...
        {editing ? (
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1">
              {editStages.map((stage, i) => (
                <div key={stage} className="flex items-center gap-2 px-2 py-1 rounded text-[11px]"
            ...
            <button onClick={handleSave} className="self-start px-2 py-1 rounded text-[12px] mt-1"
              style={{ background: 'var(--color-surface-raised)', color: 'var(--color-text-primary)', border: '1px solid var(--color-border)' }}>
              Save
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-y-1 text-[11px]" style={{ color: 'var(--color-text-secondary)' }}>
            <span>Domain</span>
            <span style={{ color: 'var(--color-text-tertiary)' }}>{loop.domain || '—'}</span>
            <span>Interval</span>
            <span style={{ color: 'var(--color-text-tertiary)' }}>{loop.interval_seconds}s</span>
            <span>Cycles</span>
            <span style={{ color: 'var(--color-text-tertiary)' }}>{loop.cycle_count}</span>
            <span>Errors</span>
            <span style={{ color: loop.error_count > 0 ? '#ef4444' : 'var(--color-text-tertiary)' }}>{loop.error_count}</span>
            <span>Started</span>
            <span style={{ color: 'var(--color-text-tertiary)' }}>{loop.started_at ?? '—'}</span>
            <span className="col-span-2 mt-1" style={{ color: 'var(--color-text-secondary)' }}>Stages</span>
            <ol className="col-span-2 list-decimal list-inside" style={{ color: 'var(--color-text-tertiary)' }}>
              {loop.stages.map((s) => (<li key={s}>{s}</li>))}
            </ol>
            {loop.description && (
              <span className="col-span-2 mt-1" style={{ color: 'var(--color-text-tertiary)' }}>{loop.description}</span>
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {!isRunning ? (
          <button onClick={() => startLoop(loop.name)} className="flex items-center gap-1 px-3 py-1.5 rounded text-[12px]"
            style={{ background: 'var(--color-surface-raised)', color: '#22c55e', border: '1px solid var(--color-border)' }}>
            <Play size={12} /> Start
          </button>
        ) : (
          <button onClick={() => stopLoop(loop.name)} className="flex items-center gap-1 px-3 py-1.5 rounded text-[12px]"
            style={{ background: 'var(--color-surface-raised)', color: '#ef4444', border: '1px solid var(--color-border)' }}>
            <Square size={12} /> Stop
          </button>
        )}
        <button onClick={() => runOnce(loop.name)} className="flex items-center gap-1 px-3 py-1.5 rounded text-[12px]"
          style={{ background: 'var(--color-surface-raised)', color: 'var(--color-text-secondary)', border: '1px solid var(--color-border)' }}>
          <SkipForward size={12} /> Run Once
        </button>
        <button onClick={() => dryRun(loop.name)} className="flex items-center gap-1 px-3 py-1.5 rounded text-[12px]"
          style={{ background: 'var(--color-surface-raised)', color: 'var(--color-text-secondary)', border: '1px solid var(--color-border)' }}>
          <FlaskConical size={12} /> Dry Run
        </button>
        <button onClick={handleDelete} className="flex items-center gap-1 px-3 py-1.5 rounded text-[12px]"
          style={{ background: 'var(--color-surface-raised)', color: '#ef4444', border: '1px solid var(--color-border)' }}>
      ...
      {lastDryRun && (loop.name === (lastDryRun as Record<string, unknown>).loop_name || (lastDryRun as Record<string, unknown>).loop_name === undefined) && (
        <div className="p-4 rounded-lg" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
          ...
          <pre className="text-[10px] p-1.5 rounded overflow-auto"
            style={{ background: 'var(--color-surface-raised)', color: 'var(--color-text-tertiary)' }}>
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center gap-2 px-3 h-8 shrink-0"
        ...
        <button onClick={closeLoop} className="flex items-center gap-1 text-[11px]" style={{ color: 'var(--color-text-secondary)' }}>
          <ArrowLeft size={12} /> All Loops
        </button>
        <span className="text-[11px]" style={{ color: 'var(--color-text-tertiary)' }}>{activeLoopId}</span>
      </div>
      <div className="flex-1 overflow-auto">
        {activeLoopType === 'persistent' ? (
          loop ? <PersistentLoopDetail loop={loop} /> : (
            <div className="flex items-center justify-center h-full text-[12px]" style={{ color: 'var(--color-text-tertiary)' }}>Loop not found</div>
          )
        ) : (
          activeLoopId && <LifecycleLoopDetail id={activeLoopId} />
        )}
      </div>
    </div>
  )
...
export function LoopCanvasWorkspace(props: LoopCanvasWorkspaceProps) {
  ...
  useEffect(() => {
    useLoopCanvasStore.getState().fetchLoops()
    useLoopCanvasStore.getState().fetchStages()
  }, [])

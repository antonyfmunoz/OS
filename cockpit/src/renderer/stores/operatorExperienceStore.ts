...
function parseDexResponse(data: Record<string, unknown>): DexResponse {
  return {
    ...
    intent: (data.intent_type as string) || (data.intent as string) || '',
    summary: (data.summary as string) || '',
    current_state: (data.current_state as string) || null,
    recommended_next_action: (data.recommended_next_action as string) || null,
    confidence: (data.system_confidence as number) || (data.confidence as number) || 0,
    safety_state: (data.output_mode as string) || 'preview_only',
    packet_preview: (data.work_packet_preview as PacketPreview) || null,
    topology_preview: (data.delegation_topology_preview as TopologyPreview) || null,
    human_actions: (data.human_required_actions as DexResponse['human_actions']) || [],
    approval_gates: (data.approval_required_actions as DexResponse['approval_gates']) || [],

#!/usr/bin/env python3
"""Wave 2 Mandatory Pass 1 (final campaign) — deliberate failure/recovery driver.

Exact qualified lifecycle, single-flight:
  restart+ready → seed(clean) → dispatch_collector(CONSUMES invocation #54)
  → wait w15 → preseed → bindable-grant → write-scenario-map
  → pause-before-dispatch → inject-failure(tools-revoked-a) → start_runner
  → resume → poll → stop_runner. Teardown run separately after terminal.

Bound to exact SHA 842434dc3fa3ed16540673f5df48950ccf6a2674.
Consumes ONE field unit at dispatch_collector (invocation #54).
"""
import sys, json
...
# ---- 0. candidate deployed + healthy; restart for fresh RES hygiene ----
...
if not ready.get("ready"):
    ...
    print(json.dumps({"ok": False, "stage": "candidate_ready", "detail": ready})); sys.exit(2)
...
# ---- 2. dispatch collector (CONSUMES THE UNIT — invocation #54) ----
log("DISPATCH COLLECTOR (consumes invocation #54)")

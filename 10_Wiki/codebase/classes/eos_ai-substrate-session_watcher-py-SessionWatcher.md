...
**File:** [[eos_ai-substrate-session_watcher-py]] | **Line:** 140
...
- [[eos_ai-substrate-session_watcher-py-SessionWatcher-last_activity]]`() → float` — 
- [[eos_ai-substrate-session_watcher-py-SessionWatcher-last_reply_preview]]`() → str` — 
...
- [[eos_ai-substrate-session_watcher-py-SessionWatcher-wait_until_idle]]`(timeout, min_stable_polls) → bool` — Block until the session is truly idle (prompt visible, no streaming,
...
- [[eos_ai-substrate-session_watcher-py-SessionWatcher-_finalize_reply]]`(clean, has_prompt) → None` — Extract reply from stabilized output and transition state.

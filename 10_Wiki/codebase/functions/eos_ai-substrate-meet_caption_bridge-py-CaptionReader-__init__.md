---
type: codebase-function
file: eos_ai/substrate/meet_caption_bridge.py
line: 247
generated: 2026-05-07
---

# CaptionReader.__init__

**File:** [[eos_ai-substrate-meet_caption_bridge-py]] | **Line:** 247
**Signature:** `__init__(meeting_code) → None`

**Class:** [[eos_ai-substrate-meet_caption_bridge-py-CaptionReader]]

*No docstring.*

## Calls

- [[eos_ai-substrate-meet_caption_bridge-py-CaptionReader-_load_persisted_offset]]
- [[eos_ai-substrate-meet_caption_bridge-py-bridge_path_for]]
- [[eos_ai-substrate-meet_caption_bridge-py-sanitize_meeting_code]]

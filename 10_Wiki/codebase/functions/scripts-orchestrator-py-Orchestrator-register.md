---
type: codebase-function
file: scripts/orchestrator.py
line: 799
generated: 2026-04-12
---

# Orchestrator.register

**File:** [[scripts-orchestrator-py]] | **Line:** 799
**Signature:** `register(job) → None`

**Class:** [[scripts-orchestrator-py-Orchestrator]]

*No docstring.*

## Calls

- [[scripts-orchestrator-py-ActivityLog-emit]]
- [[scripts-orchestrator-py-Verifier-validate_job]]

## Called By

- [[core-control_plane-py-ControlPlane-__init__]]
- [[core-control_plane-py-ControlPlane-register_job]]
- [[scripts-orchestrator-py-_cmd_list]]
- [[scripts-orchestrator-py-_cmd_start]]
- [[scripts-orchestrator-py-_cmd_trigger]]

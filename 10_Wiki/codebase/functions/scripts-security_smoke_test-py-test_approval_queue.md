---
type: codebase-function
file: scripts/security_smoke_test.py
line: 166
generated: 2026-04-12
---

# test_approval_queue

**File:** [[scripts-security_smoke_test-py]] | **Line:** 166
**Signature:** `test_approval_queue(tmp) → None`

*No docstring.*

## Calls

- [[core-security-approval-py-ApprovalQueue-approve]]
- [[core-security-approval-py-ApprovalQueue-create_request]]
- [[core-security-approval-py-ApprovalQueue-reject]]
- [[core-security-context-py-SecurityContext-approve]]
- [[core-security-context-py-SecurityContext-reject]]
- [[scripts-security_smoke_test-py-assert_eq]]
- [[scripts-security_smoke_test-py-step]]

## Called By

- [[scripts-security_smoke_test-py-main]]

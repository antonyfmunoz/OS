---
type: codebase-function
file: scripts/substrate_google_meet_smoke_test.py
line: 69
generated: 2026-04-12
---

# main

**File:** [[scripts-substrate_google_meet_smoke_test-py]] | **Line:** 69
**Signature:** `main() → int`

*No docstring.*

## Calls

- [[eos_ai-substrate-audio_loop-py-AudioLoopStore-clear]]
- [[eos_ai-substrate-audio_loop-py-AudioLoopStore-get]]
- [[eos_ai-substrate-audio_loop-py-get_audio_loop_store]]
- [[eos_ai-substrate-audio_loop-py-reset_audio_loop_store_for_tests]]
- [[eos_ai-substrate-google_meet_source-py-GoogleMeetSource-attach_hook]]
- [[eos_ai-substrate-google_meet_source-py-GoogleMeetSource-close]]
- [[eos_ai-substrate-google_meet_source-py-GoogleMeetSource-read_utterance]]
- [[eos_ai-substrate-google_meet_source-py-GoogleMeetSource-status_snapshot]]
- [[eos_ai-substrate-google_meet_source-py-is_google_meet_source]]
- [[eos_ai-substrate-google_meet_source-py-parse_meet_url]]
- [[eos_ai-substrate-meeting_sources-py-FakeMeetingSource-close]]
- [[eos_ai-substrate-meeting_sources-py-FakeMeetingSource-read_utterance]]
- [[eos_ai-substrate-meeting_sources-py-LiveMeetingSourceStub-close]]
- [[eos_ai-substrate-meeting_sources-py-LiveMeetingSourceStub-read_utterance]]
- [[eos_ai-substrate-meeting_sources-py-MeetingSourceProtocol-close]]
- [[eos_ai-substrate-meeting_sources-py-MeetingSourceProtocol-read_utterance]]
- [[eos_ai-substrate-meeting_sources-py-is_meeting_source]]
- [[eos_ai-substrate-meeting_transport-py-MeetingTransport-attach_source]]
- [[eos_ai-substrate-meeting_transport-py-MeetingTransport-detach_source]]
- [[eos_ai-substrate-meeting_transport-py-MeetingTransport-inject_utterance]]

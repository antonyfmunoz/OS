---
type: codebase-function
file: scripts/orchestrator.py
line: 245
generated: 2026-05-07
---

# ActivityLog.emit

**File:** [[scripts-orchestrator-py]] | **Line:** 245
**Signature:** `emit(event) → None`

**Class:** [[scripts-orchestrator-py-ActivityLog]]

*No docstring.*

## Calls

- [[scripts-workflow_engine-py-AgentRegistry-get]]

## Called By

- [[scripts-orchestrator-py-EventAgent-start]]
- [[scripts-orchestrator-py-ExecutionQueue-_run_job]]
- [[scripts-orchestrator-py-ExecutionQueue-start]]
- [[scripts-orchestrator-py-ExecutionQueue-stop]]
- [[scripts-orchestrator-py-ExecutionQueue-submit]]
- [[scripts-orchestrator-py-Orchestrator-register]]
- [[scripts-orchestrator-py-Orchestrator-start]]
- [[scripts-orchestrator-py-Orchestrator-stop]]
- [[scripts-orchestrator-py-Orchestrator-submit]]
- [[scripts-orchestrator-py-Orchestrator-unregister]]
- [[scripts-orchestrator-py-RetryPolicy-_handle]]
- [[scripts-orchestrator-py-SchedulerAgent-_loop]]
- [[scripts-orchestrator-py-SchedulerAgent-tick_once]]

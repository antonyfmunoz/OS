...
All symbols are available via ``from execution.transport import X`` or
``from execution.transport.submodule import Y``. Submodules are loaded
...
def _m(submodule: str, *names: str) -> None:
    ...
    for name in names:
        _LAZY_MAP[name] = (f"execution.transport.{submodule}", name)

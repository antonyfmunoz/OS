...
def process_task(
    ...
    *,
    ...
    try:
        from execution.transport.task_queue import prioritize_and_queue
    ...
    if task.execution_policy == TaskExecutionPolicy.AUTONOMOUS:
        if is_day_open:
            if use_v2_execution:
                ...
                try:
                    from execution.transport.task_execution import execute_task
...
def run_overnight_tasks(
    *,
    ...
    if use_v2_execution:
        try:
            from execution.transport.task_execution import run_overnight_execution

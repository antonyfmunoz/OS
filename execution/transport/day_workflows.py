...
from execution.transport.operator_session import (
    OperatorDayMode,
    OperatorSession,
    OperatorSessionStore,
)
from execution.transport.rituals import RitualKind, RitualRegistry, RitualState
...
def open_day(
    *,
    ...
    try:
        from execution.transport.station_presence import (
            StationPresenceMode,
            set_presence_mode,
        )
    ...
    task_summary: dict = {}
    try:
        from execution.transport.task_queue import get_enhanced_task_summary

        task_summary = get_enhanced_task_summary()
    except Exception:  # noqa: BLE001
        # v2 unavailable — fall back to v1 summary
        try:
            from execution.transport.task_system import get_task_summary
    ...
    try:
        from execution.transport.pipeline_execution import get_pipeline_summary
    ...
    try:
        from execution.transport.auto_task_generation import get_perception_summary
    ...
    try:
        from execution.transport.live_sessions import get_live_session_summary
    ...
    try:
        from execution.transport.station_presence import get_station_summary
    ...
    node_health: dict = {}
    try:
        from execution.transport.node_controller import get_node_health_summary
    ...
    try:
        from execution.transport.task_queue import get_waiting_on_operator_tasks
...
def close_day(
    *,
    ...
    try:
        from execution.transport.scenes import get_scene as _get_scene
    ...
    if day_mode == OperatorDayMode.OVERNIGHT:
        try:
            from execution.transport.task_queue import prepare_overnight_queue
    ...
    if day_mode == OperatorDayMode.OVERNIGHT:
        try:
            from execution.transport.task_system import run_overnight_tasks
    ...
    try:
        from execution.transport.station_presence import get_station_presence

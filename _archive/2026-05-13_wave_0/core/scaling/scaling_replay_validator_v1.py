"""Scaling Replay Validator v1.

Replays:
  pressure regulation, throttling decisions,
  concurrency arbitration, degraded-mode transitions,
  priority arbitration.

Verifies:
  same pressure state → same scaling decisions.
...
REPLAY_CHECKS: list[str] = [
    ...
    "concurrency_arbitration",
    "degraded_mode_transitions",
    "priority_arbitration",
]


class ScalingReplayValidator:
    """Validates deterministic replay of scaling decisions."""

    def __init__(
        self,
        state_dir: str | Path = "data/runtime/scaling/replay",
    ) -> None:
    ...
    def validate_pressure_determinism(
        self,
        snapshots_a: list[dict[str, Any]],
        snapshots_b: list[dict[str, Any]],
    ) -> dict[str, Any]:
        hash_a = _content_hash(snapshots_a)
        hash_b = _content_hash(snapshots_b)
        passed = hash_a == hash_b
        self._total_validations += 1
        if passed:
            self._total_passes += 1
        result = {
            "check": "pressure_determinism",
            "hash_a": hash_a, "hash_b": hash_b,
            "passed": passed, "timestamp": _now_iso(),
        }
        self._persist_proof(result)
        return result

    def validate_arbitration_determinism(
        self,
    ...
    def validate_degraded_determinism(
        self,
        decisions_a: list[dict[str, Any]],
        decisions_b: list[dict[str, Any]],
    ) -> dict[str, Any]:
    ...
    def _persist_proof(self, proof: dict[str, Any]) -> None:
        path = self._state_dir / "scaling_replay_proofs.jsonl"

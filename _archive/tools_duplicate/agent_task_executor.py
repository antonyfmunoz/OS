...
def execute_agent_task(task: dict, ctx) -> dict:
    """
    Execute a single agent task through the canonical ExecutionSpine.
    ...
    _tt_val = getattr(task_type, "value", str(task_type)).lower()
    authority_class = _authority_map.get(_tt_val, "analyze")
...
async def run_executor():
    ...
    agent_tasks = [
        ...
        if t.get("assignee_type") == "agent" and t.get("assignee_id") in AGENT_MAP
    ][:MAX_TASKS_PER_RUN]
    ...
    for task in agent_tasks:
        ...
        try:
            from umh.runtime_engine.notion_sync import write_task
            from umh.storage.adapters.neon import get_conn
            ...
            agent_cfg = AGENT_MAP.get(exec_result.get("agent_id", ""), {})
            notion_page_id = write_task(
                venture_id=venture_id,
                name=(f"[{agent_cfg.get('display', 'Agent')}] {description[:120]}"),
        ...
        results_for_discord.append(
            {
                "task_id": task_id,
                "description": description,
                "agent_name": exec_result.get(
                    "display_name", exec_result.get("agent_id", "")
                ),

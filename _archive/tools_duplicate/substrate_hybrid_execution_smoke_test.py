...
def test_target_policy_defaults() -> None:
    ...
    from umh.substrate.target_policy import resolve_execution_target
...
def test_resolve_mode_session_builder_local() -> None:
    ...
    from umh.substrate.discord_mode_routing import resolve_mode_session
...
def test_no_hot_path_imports_target_policy() -> None:
    ...
    with open("/opt/OS/eos/substrate/target_policy.py") as f:
...
def test_no_hot_path_imports_mode_routing() -> None:
    ...
    with open("/opt/OS/eos/substrate/discord_mode_routing.py") as f:
        tree = ast.parse(f.read())
...
def test_one_router_invariant() -> None:
    ...
    for path in (
        "/opt/OS/eos/substrate/target_policy.py",
        "/opt/OS/eos/substrate/discord_mode_routing.py",
    ):
        with open(path) as f:
            tree = ast.parse(f.read())

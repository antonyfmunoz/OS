...
Wraps MediaProcessor synthesis/transcription into a clean interface for:
  - Full voice conversation turns (transcribe → ExecutionSpine → synthesize)
  - Meeting session capture (accumulate transcript without synthesis)
  - Meeting analysis (structured extraction via ExecutionSpine ANALYZE)
...
class VoiceInterface:
    ...
    def _spine_call(
        ...
        """Route a text prompt through ExecutionSpine. Returns response string."""
    ...
    def process_voice_turn(
        ...
        # 2. Route through ExecutionSpine (pure text — transcription already done)
        response_text = (
            self._spine_call(transcript, agent=agent, venture_id=venture_id) or ""
        )
    ...
    def get_meeting_brief(
        ...
        """
        ctx_map = self.MEETING_CONTEXTS.get(
            meeting_type, self.MEETING_CONTEXTS["sales_call"]
        )
        agent = ctx_map["pre_brief_agent"]
        agenda = "\n".join(
            f"  {i + 1}. {item}" for i, item in enumerate(ctx_map["agenda_items"])
        )
        ...
        try:
            brief = self._spine_call(prompt, agent=agent, venture_id=venture_id) or ""
    ...
    def get_during_meeting_context(
        ...
        if q in ("tasks", "todo", "action items"):
            try:
                ...
                for r in rows:
                    lines.append(
                        f"  [{r['priority'] or '—'}] {r['title']} ({r['status']})"
                    )
                return "\n".join(lines)
            except Exception as e:
        ...
        if q in ("objections", "objection", "pushback"):
            ...
            try:
                return (
                    self._spine_call(prompt, agent="sales_agent", venture_id=venture_id)
                    or "No objections generated."
                )
            except Exception as e:
        ...
        try:
            return (
                self._spine_call(prompt, agent=agent, venture_id=venture_id)
    ...
    def end_meeting_session(self, session_id: str) -> dict:
        ...
        """
        full_text = "\n".join(
            entry["text"] for entry in self._session_transcript if entry.get("text")
        ).strip()
        ...
        output = (
            self._spine_call(prompt, agent="ceo_agent", venture_id="lyfe_institute")
            or ""
        )
        ...
        if action_items:
            try:
                ...
                action_text = "\n".join(
                    f"- {a['owner']}: {a['action']}" for a in action_items
                )
                ce.ceo_delegate(
                    company_objective=(

...
STAGE_TRANSITIONS: dict[int, dict] = {
    1: {
        ...
        "signal": ("First paying client acquired from consistent channel"),
    },
    2: {
        ...
        "signal": ("10 sales from same channel with same message"),
    },
    3: {
        ...
        "signal": ("Consistent monthly revenue without founder direct involvement"),
    },
}
...
class EvolutionEngine:
    ...
    def is_primitive_unlocked(
        self, primitive_id: str, venture_id: str = "lyfe_institute"
    ) -> dict:
        """
        ...
        """
        from umh.runtime_engine.primitives import PRIMITIVE_LIBRARY, STAGE_PRIMITIVES
    ...
    def analyze_system_performance(self) -> dict:
            ...
            with get_conn(self.ctx.org_id) as cur:
                ...
                if row:
                    ...
                    if total_outcomes_30d > 0:
                        overall_reply_rate = round(replies / total_outcomes_30d, 3)
        ...
        try:
            ...
            if iteration_values:
                avg_iterations = round(sum(iteration_values) / len(iteration_values), 2)
    ...
    def propose_workflow_improvement(self, workflow_id: str) -> dict:
        ...
        try:
            ...
            _uctx = ContextBuilder().build(
                self.ctx, prompt, _sid, agent="evolution_engine.workflow"
            )
            _spine_out = run_via_umh(
                message=prompt,
                unified_context=_uctx,
                agent_type="evolution_engine.workflow",
                authority_class="analyze",
                session_id=_sid,
                org_id=str(self.ctx.org_id),
                task_type=TaskType.ANALYZE,
            )
            raw = (_spine_out or "").strip()
            if raw.startswith("```"):
                import re as _re
        ...
        try:
            ...
            print(
                f"[EvolutionEngine] Workflow improvement queued → approval {approval_id}"
            )
        except Exception as e:
            return {
                "status": "error",
    ...
    def propose_new_agent(self, pattern_description: str) -> dict:
        ...
        _v_name = (
            _active.get("name", "the active venture")
            if isinstance(_active, dict)
            else "the active venture"
        )
        ...
        try:
            ...
            _sid = str(_uuid.uuid4())
            _uctx = ContextBuilder().build(
                self.ctx, prompt, _sid, agent="evolution_engine.agent_designer"
            )
            _spine_out = run_via_umh(
                message=prompt,
                unified_context=_uctx,
                agent_type="evolution_engine.agent_designer",
        ...
        try:
            ...
            print(
                f"[EvolutionEngine] New agent proposed: {agent_spec.get('name')} → approval {approval_id}"
            )
        except Exception as e:
            return {
                "status": "error",
    ...
    def run_weekly_evolution_cycle(self) -> dict:
        ...
        try:
            ...
            print(
                f"[EvolutionEngine] Skills: {len(improved)} improved of {len(skill_results)} checked"
            )
        except Exception as e:
        ...
        try:
            ...
            summary["knowledge_objects_created"] = gap_result.get(
        ...
        if perf["overall_reply_rate"] is not None and perf["overall_reply_rate"] < 0.25:
            try:
                with get_conn(self.ctx.org_id) as cur:
                    cur.execute(
        ...
        try:
            ...
            for pattern in patterns:
                ...
                print(
                    f"[EvolutionEngine] Agent proposal: {result.get('name')} — {result['status']}"
                )
        except Exception as e:
    ...
    def format_evolution_summary(self, summary: dict) -> str:
        ...
        return (
            ...
            + (
                f"\n\nErrors: {summary.get('skills_error', '')}"
                if summary.get("skills_error")
                else ""
            )
        )

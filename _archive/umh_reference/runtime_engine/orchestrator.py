...
Usage (manual):
    python3 umh/orchestrator.py

Cron (6am daily):
    0 6 * * * cd /opt/OS && python3 umh/orchestrator.py >> logs/orchestrator.log 2>&1
"""
...
from umh.storage.adapters.neon import get_conn, resolve_venture
...
def _notify(text: str) -> None:
    ...
    try:
        from umh.actions.channel import get_channel_router
...
class CEOAgent:
    ...
    def delegate_objective(self, objective: str, venture_id: str) -> dict:
        ...
        try:
            ...
            pass
        from umh.runtime_engine.gateway import get_gateway as _get_gw
...
def run_full_morning_cycle(ctx: EOSContext, return_content: bool = False):
    ...
    try:
        from umh.runtime_engine.claude_skill_registry import ClaudeSkillRegistryManager
    ...
    try:
        from umh.runtime_engine.ceo_agent import CEOAgent as _CEOEvolutionAgent
    ...
    notion_url = ""
    try:
        from umh.runtime_engine.notion_publisher import get_publisher
...
def run_ceo_morning_delegation(
    ...
    try:
        from umh.runtime_engine.ceo_agent import CEOAgent as _EvoCEO
    except ImportError:
        pass
    try:
        from umh.runtime_engine.coordination_engine import CoordinationEngine as _CE
    except ImportError:
        pass
    try:
        from umh.runtime_engine.portfolio_advisor import PortfolioAdvisor as _PA
    except ImportError:
        pass
    ...
    for venture_config in venture_list[:3]:
        ...
        try:
            ...
            from umh.environments.system_context import EOSContext as _EC
            ...
            constraint_context = ""
            try:
                from umh.runtime_engine.ceo_intelligence import (
...
async def generate_morning_brief(ctx: EOSContext) -> str:
    ...
    if not _ctx_ventures:
        try:
            from umh.environments.system_context import load_context_from_env as _lctx
    ...
    for venture_id, name, icon in companies:
        try:
            ...
            from umh.runtime_engine.primitives import PRIMITIVE_LIBRARY
    ...
    reality_section = ""
    try:
        from umh.runtime_engine.reality_context import RealityContextEngine
    ...
    try:
        from umh.runtime_engine.gateway import get_gateway as _get_gw_brief
...
class EOSOrchestrator:
    ...
    def morning_brief(self) -> str:
        ...
        try:
            from umh.environments.system_context import load_context_from_env as _lctx
            from umh.analytics.pattern_engine import PatternEngine as _PE
    ...
    def run_morning_cycle(self) -> None:
        ...
        if datetime.date.today().weekday() == 5:  # Saturday = 5
            try:
                from umh.runtime_engine.embedding_engine import EmbeddingEngine
                from umh.environments.system_context import load_context_from_env
...
def refresh_ambient_state(ctx: EOSContext) -> None:
    ...
    try:
        ...
        from umh.runtime_engine.session_state import SessionState
...
def start_ambient_refresh_loop(ctx: EOSContext) -> None:
    ...
    def _refresh_loop() -> None:
        while True:
            ...
            try:
                from umh.runtime_engine.proactive_engine import ProactiveIntelligenceEngine

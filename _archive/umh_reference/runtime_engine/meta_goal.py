...
Usage::
    ...
    from umh.runtime_engine.meta_goal import MetaGoalEngine, MetaGoal
...
class MetaGoalEngine:
    ...
    def _decay_generated(self, registry: GoalRegistry, current_turn: int) -> None:
        ...
        for gid, mg in list(self._generated_goals.items()):
            ...
            staleness = current_turn - (
                tracker.last_active_turn if tracker else mg.generation_turn
            )
    ...
    def _can_generate(self, registry: GoalRegistry, current_turn: int) -> bool:
        ...
        generated_active = sum(
            1
            for mg in self._generated_goals.values()
            if mg.lifecycle_state != "retired"
        )
    ...
    def _check_triggers(
        ...
        if not generated:
            ...
            if high_ent:
                split_goals = self._generate_specializations(
                    registry, traces, current_turn
                )
    ...
    def snapshot(self) -> dict:
        ...
        return {
            "generated_goals": {
                gid: mg.to_dict() for gid, mg in self._generated_goals.items()
            },

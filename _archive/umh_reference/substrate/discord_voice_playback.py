...
2. Reuses `umh.voice_engine.VoiceEngine.speak()` (lazy import) — no
...
def probe_playback_capability() -> dict[str, Any]:
    ...
    try:
        ...
        importlib.import_module("umh.voice_engine")
...
def _render_tts_to_wav(text: str) -> Optional[str]:
    ...
    Strategy:
      1. Try `umh.voice_engine.VoiceEngine.speak()` (Coqui TTS → espeak).
    ...
    try:
        from umh.runtime_engine.voice_engine import VoiceEngine  # type: ignore

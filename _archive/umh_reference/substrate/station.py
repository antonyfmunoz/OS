...
Design rules:
  - No raw remote shell. Only SafeAction intents (see umh.substrate.actions).
...
Usage (contract consumer side — substrate, not daemon):
    from umh.world.substrate import StationContract, StationHeartbeat
...
from umh.substrate.actions import SafeAction, ActionResult, ActionKind, ActionStatus
...
class StationContract:
    ...
    def record_event(self, evt: StationEvent) -> None:
        ...
        try:
            from umh.runtime_engine.event_bus import EventBus
    ...
    def propose(self, action: SafeAction) -> SafeAction:
        ...
        (see umh.substrate.station_bus). Never raises on transport errors —
        ...
        try:
            from umh.substrate.station_bus import get_station_bus

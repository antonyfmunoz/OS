...
Usage:
    from umh.workstation.business import BusinessInstance, BusinessInstanceManager
...
This is the UMH-owned extraction of umh/business_instance.py.
"""
...
STAGE_GUIDANCE: dict[int, dict[str, Any]] = {
    1: {
...
class BusinessInstance:
    """Venture-stage context for a single company/venture."""
    ...
    ai_soul_doc_path: str = ""

    os_subscriptions: list = field(default_factory=lambda: ["entrepreneur_os"])
...
class BusinessInstanceManager:
    """Persist and retrieve BusinessInstance data via the ventures table config_json column."""
    ...
    def get_default_venture_id(self) -> Optional[str]:
        """Return the default venture_id for the current org.
    ...
    def get_stage_guidance(self, venture_id: str) -> dict[str, Any]:
    ...
    def create_from_wizard(self, answers: dict[str, Any]) -> BusinessInstance:
        """Create a BusinessInstance from onboarding wizard answers.

        Uses ExecutionSpine to fill any missing gaps.
        ...
        if gaps:
            try:
                ...
                output = utility_llm_call(
                    prompt=gap_prompt,
                    system="Return ONLY valid JSON with the requested fields.",
                    operation="bis_gap_fill",
                )

                try:
                    filled = json.loads(output or "{}", strict=False)
                    for gap_field in gaps:
                        if gap_field in filled and filled[gap_field]:
                            setattr(bis, gap_field, filled[gap_field])
                except Exception:
                    pass
            except Exception as e:
    ...
    def advance_stage(self, venture_id: str, proof: dict[str, Any]) -> dict[str, Any]:
    ...
    def format_full_summary(self, venture_id: str) -> str:
        """Return full BIS summary for /bis command."""
        bis = self.get_bis(venture_id)
        if not bis:
...
def get_ai_name(ctx: Any, venture_id: str = "lyfe_institute") -> str:
